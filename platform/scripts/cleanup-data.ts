...
async function cleanup() {
  ...
  console.log(
    `🧹 Starting Data Cleanup (${isDryRun ? "DRY RUN" : "EXECUTION MODE"})...`,
  );
  ...
  console.log(
    `- Found ${sessions} stale sessions (older than ${sessionCutoff.toISOString()})`,
  );
  ...
  try {
    ...
    console.log(
      `- Found ${auditLogs} old audit logs (older than ${auditCutoff.toISOString()})`,
    );
