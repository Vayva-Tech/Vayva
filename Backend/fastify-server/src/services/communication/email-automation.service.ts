 ...
 * Sends automated reports and notifications using Resend.com
 */
export class EmailAutomationService {
  ...
  /**
   * Send weekly/monthly report to client with retry logic
   */
  async sendClientReport(
    clientEmail: string,
    data: ReportData,
  ): Promise<{ success: boolean; error?: string }> {
    return this.sendWithRetry('report', async () => {
      try {
        const html = this.generateReportHTML(data);
        const text = this.generateReportText(data);
        ...
        logger.info(`[Email] Report sent to ${clientEmail}`);
        return { success: true };
      } catch (error) {
        logger.error('[Email] Failed to send report', { error });
        throw error;
      }
    });
  }
  ...
  async sendMilestoneNotification(
    ...
    try {
      ...
      logger.info(`[Email] Milestone notification sent to ${clientEmail}`);
      return { success: true };
    } catch (error) {
      logger.error('[Email] Failed to send milestone notification', { error });
      return {
        success: false,
  ...
  async sendInvoiceReminder(
    ...
    try {
      ...
      logger.info(`[Email] Invoice reminder sent to ${clientEmail}`);
      return { success: true };
    } catch (error) {
      logger.error('[Email] Failed to send invoice reminder', { error });
      return {
        success: false,
        error: error instanceof Error ? error.message : 'Unknown error',
      };
   ...
   */
  private generateReportText(data: ReportData): string {
    return `
...
Powered by Vayva
  ...
  private async sendWithRetry<T>(
    ...
    for (let attempt = 0; attempt <= config.maxRetries; attempt++) {
      try {
        ...
        if (attempt > 0) {
          logger.info(`[Email] Sent successfully on attempt ${attempt + 1}`);
        }
        ...
        logger.error(
          `[Email] Send failed (attempt ${attempt + 1}/${config.maxRetries + 1})`,
          { error: lastError.message },
        );

        if (attempt < config.maxRetries) {
          const delay = this.calculateExponentialDelay(attempt, config);
          logger.warn(`[Email] Retrying in ${delay}ms...`);
          await this.sleep(delay);
        }
      }
    }

    logger.error(`[Email] ${type} failed after ${config.maxRetries + 1} attempts`);
    await this.logFailedEmail(type, lastError?.message || 'Unknown error');
   ...
   */
  private calculateExponentialDelay(attempt: number, config: RetryConfig): number {
   ...
   */
  private async logFailedEmail(type: string, error: string): Promise<void> {
    try {
      logger.warn('[Email] Logged failure to database', { type, error });
    } catch (logError) {
      logger.error('[Email] Failed to log failure to database', { error: logError });
    }
  }
}
