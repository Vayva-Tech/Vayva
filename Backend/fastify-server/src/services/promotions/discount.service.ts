...
export class DiscountService {
   ...
   */
  async createDiscount(storeId: string, data: Record<string, unknown>) {
    ...
    return await this.db.$transaction(async (tx) => {
      ...
      logger.info(`[Discount] Created discount rule ${rule.id}${coupon ? ` with coupon ${code}` : ''}`);
  ...
  /**
   * Get discount details by rule ID
   */
  async getDiscount(storeId: string, ruleId: string) {
    const rule = await this.db.discountRule.findUnique({
  ...
  /**
   * Update discount rule and sync coupon
   */
  async updateDiscount(
    ...
    return await this.db.$transaction(async (tx) => {
      ...
      // 2. Sync coupon if code provided
      ...
      logger.info(`[Discount] Updated discount rule ${ruleId}`);
  ...
  /**
   * Delete discount rule and associated coupons
   */
  async deleteDiscount(storeId: string, ruleId: string) {
    return await this.db.$transaction(async (tx) => {
      ...
      logger.info(`[Discount] Deleted discount rule ${ruleId}`);
    });
  }
}
