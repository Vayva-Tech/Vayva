...
export function registerWhatsAppInboundWorker(
      ...
      try {
        ...
        const message = await prisma.message.create({
          data: {
            ...
            type:
              (messageData.type?.toUpperCase() as MessageType) ||
              MessageType.TEXT,
        ...
        await WorkerRescueService.reportJobFailure(
          QUEUES.WHATSAPP_INBOUND,
          job.id!,
          error,
        );
  ...
  logger.info("Registered WhatsApp inbound worker", {
    queue: QUEUES.WHATSAPP_INBOUND,
    app: "worker",
  });
}
