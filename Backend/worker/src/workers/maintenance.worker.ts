...
export function registerMaintenanceWorker(connection: RedisConnection): void {
  new Worker<MaintenanceJobData>(
    ...
    async () => {
      ...
      await drainEmailOutbox().catch((err) =>
        logger.error("drainEmailOutbox failed", {
          error: err.message,
          app: "worker",
        }),
      );
      await drainNotificationOutbox().catch((err) =>
        logger.error("drainNotificationOutbox failed", {
          error: err.message,
          app: "worker",
        }),
      );
  ...
  logger.info("Registered maintenance worker", {
    queue: QUEUES.MAINTENANCE_CLEANUP,
    app: "worker",
  });
}
