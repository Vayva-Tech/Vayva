...
export async function sendTrackingNotification(
  ...
  try {
    ...
    if (!EVOLUTION_API_URL || !EVOLUTION_API_KEY) {
      logger.warn("[Worker TrackingNotification] Evolution API not configured", {
    ...
    logger.info("[Worker TrackingNotification] WhatsApp notification sent", {
    ...
    logger.error("[Worker TrackingNotification] Failed to send notification", { 
      error: errorMessage,
    });
    return {
      success: false,
...
export async function sendDeliveryStatusUpdate(
  ...
  try {
    ...
    logger.info("[Worker TrackingNotification] Status update sent", {
      phone: formattedPhone.replace(/\d(?=\d{4})/g, "*"),
      trackingCode,
      status,
    });
    ...
    logger.error("[Worker TrackingNotification] Failed to send status update", { error: errorMessage });
    return {
      success: false,
      error: errorMessage,
    };
