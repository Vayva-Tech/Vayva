...
export function AddOnsSection() {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-12">
        <Loader2 className="animate-spin w-6 h-6 text-text-tertiary" />
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-bold text-text-primary">Add-ons</h2>
        <p className="text-sm text-text-secondary mt-1">
          Expand your dashboard with extra industry modules. Each add-on is{" "}
          <span className="font-bold text-text-primary">₦10,000/month</span> and
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {purchasable.map((addOn) => {
          const isActive = addOn.purchase?.status === "ACTIVE";
