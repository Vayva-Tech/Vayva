...
import { ProductServiceItem, ProductServiceType } from "@vayva/shared";
...
export const ProductList: React.FC<ProductListProps> = ({
  ...
  if (items.length === 0) {
    return (
      <div className="p-16 text-center text-text-tertiary flex flex-col items-center">
        <div className="w-20 h-20 bg-white/40 rounded-full flex items-center justify-center mb-6 ring-8 ring-gray-50/50">
          <Icon name="Package" size={40} className="text-text-tertiary" />
        </div>
  ...
  const columns: Column<ProductServiceItem>[] = [
    {
      ...
      render: (item) => (
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 md:w-12 md:h-12 rounded-lg bg-white/40 border border-border flex items-center justify-center shrink-0 overflow-hidden">
            {item.images?.[0] ? (
              <img
                src={item.images[0]}
                alt={item.name}
              ...
              <Icon name="Image" size={20} className="text-text-tertiary" />
            )}
          </div>
      ...
      render: (item) => (
        ...
        >
          <Button
            variant="ghost"
            size="icon"
            className="h-8 w-8 text-text-tertiary hover:text-black touch-target-sm"
            onClick={() => onEdit(item)}
          ...
          <Button
            variant="ghost"
            size="icon"
            className="h-8 w-8 text-text-tertiary hover:text-red-600 hover:bg-red-50 touch-target-sm"
  ...
  return (
    <ResponsiveTable<ProductServiceItem>
      data={items}
      columns={columns}
