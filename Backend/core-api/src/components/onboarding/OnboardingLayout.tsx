      ...
      return (
        <div
          ...
          className={cn(
            ...
            isCurrent ? "bg-vayva-green/10" : "hover:bg-white/40",
          )}
            ...
            {isCompleted ? (
              ...
              <>
                {step.icon === "Sparkles" && (
                  <Sparkles
                    size={14}
                    className={isCurrent ? "text-white" : "text-text-tertiary"}
                  />
                )}
                {step.icon === "User" && (
                  <User
...
export default function OnboardingLayout({
  ...
  const merchantName = useMemo(() => {
    ...
    const maybe = m?.firstName || m?.businessName || m?.storeName;
    return (maybe as string) || "Merchant";
  }, [merchant]);

  useEffect(() => {
    // Toggle based on state change
  }, [currentStep]);
      ...
      <div className="relative flex min-h-screen bg-background/40 backdrop-blur-[2px]">
        <div className="hidden lg:flex lg:w-80 xl:w-[340px] bg-background/70 backdrop-blur-xl border-r border-border/70 px-6 py-6 flex-col fixed left-0 top-0 bottom-0">
          <div className="mb-6 flex items-center gap-3">
            <div className="h-10 w-10 rounded-2xl bg-black flex items-center justify-center overflow-hidden">
              <Image
          ...
          <div className="mt-6 p-4 bg-white/40 rounded-2xl border border-border/60">
          ...
          <div className="sticky top-0 z-30 border-b border-border/60 bg-background/70 backdrop-blur-xl">
            <div className="px-4 sm:px-6 lg:px-10 py-4 flex items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-2">
                <Button
                  variant="outline"
                  className="h-10 rounded-xl border-border bg-background hover:bg-white/40 text-text-secondary"
                  onClick={() => {
