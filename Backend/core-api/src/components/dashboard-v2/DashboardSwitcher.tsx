...
export function DashboardSwitcher({
    ...
    const fetchAddOns = async () => {
      try {
        ...
        const active = (data.addOns || [])
          .filter((a) => a.purchase?.status === "ACTIVE" && !a.isBaseExtension)
          .map((a) => ({
            extensionId: a.id,
            name: a.name,
            icon: a.icon || "Blocks",
  ...
  return (
    <div className="flex items-center gap-1 p-1 rounded-2xl bg-white/30 border border-border/40 backdrop-blur-sm">
      {tabs.map((tab) => {
