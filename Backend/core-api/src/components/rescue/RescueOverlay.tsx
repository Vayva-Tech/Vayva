...
export function RescueOverlay({ error, reset }: RescueOverlayProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-background/80 backdrop-blur-md flex items-center justify-center p-4">
      <div className="bg-background rounded-3xl shadow-2xl border border-border/40 max-w-md w-full overflow-hidden p-8 text-center animate-in fade-in zoom-in duration-300">
        <div className="flex justify-center mb-6">
          <div
            className={`p-4 rounded-full ${status === "RUNNING" ? "bg-indigo-50 animate-pulse" : "bg-green-50"}`}
          >
        ...
        <p className="text-text-tertiary mb-8">{statusMessage}</p>

        <div className="space-y-3">
          {status === "READY_TO_REFRESH" || status === "NEEDS_ENGINEERING" ? (
            <Button
              onClick={handleRefresh}
              className="w-full h-12 text-base font-bold bg-text-primary hover:bg-text-primary text-white rounded-xl transition-all"
            >
              <RefreshCw className="w-4 h-4 mr-2" />
              Refresh Page
            </Button>
          ) : (
            <div className="h-12 flex items-center justify-center text-sm font-semibold text-text-tertiary">
              Working on it...
            </div>
          )}
        ...
        {incidentId && (
          <div className="mt-8 pt-4 border-t border-border/20 text-[10px] text-text-tertiary uppercase tracking-widest">
            Rescue Incident ID:{" "}
