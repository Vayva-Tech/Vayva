...
&nbsp;</p>
  ...
  // --- 7. Security Alert (Suspicious Login) ---
  static async sendSecurityAlertEmail(
    ...
    if (!FEATURES.EMAIL_ENABLED && process.env.NODE_ENV !== "production") {
      logger.warn(
        "[ResendEmailService] EMAIL_ENABLED=false. Skipping security alert email in development.",
        { app: "merchant" },
      );
      return { success: true, skipped: true };
    }
    ...
    try {
      const { data, error } = await this.client.emails.send({
        ...
        html: wrapEmail(
          this.getSecurityAlertTemplate(firstName, alertType, ipAddress, userAgent, timestamp),
          "Security Alert",
        ),
      });
      if (error) {
        logger.error("[Resend] Security Alert Error:", {
          error: error instanceof Error ? error.message : String(error),
        });
        throw new Error(
  ...
  static getSecurityAlertTemplate(
    firstName: string,
    alertType: string,
    ipAddress: string,
    userAgent: string,
    ...
    const alertMessages: Record<string, string> = {
      new_device: "We noticed a login from a new device.",
      new_location: "We noticed a login from a new location.",
      multiple_failures: "Multiple failed login attempts were detected.",
      unusual_time: "We noticed a login at an unusual time.",
      ip_reputation: "We noticed suspicious activity from your IP address.",
    };
    const securityUrl = urls.merchantDashboard();
    return `
            <h1 style="margin:0 0 12px; font-size:22px; font-weight:600;">
                Security Alert
            </h1>
            <p style="margin:0 0 16px; font-size:16px; line-height:1.6; color:#444444;">
                Hi <strong>${firstName}</strong>, ${alertMessages[alertType] || "We detected suspicious activity on your account."}
            </p>
            ...
            <div style="background:#fff1f2; border-radius:8px; padding:16px; margin:24px 0; color:#be123c; font-size:14px; line-height:1.5;">
                ...
                <strong>Device:</strong> ${userAgent.slice(0, 50)}...
            </div>
            
            ${renderButton(securityUrl, "Review Account Activity")}
