...
interface OnboardingLogistics {
  deliveryMode?: string;
  pickupAddress?: string;
}

interface OnboardingStoreDetails {
  slug?: string;
  domainPreference?: string;
  publishStatus?: string;
}

interface OnboardingWhatsapp {
  number?: string;
}
...
interface OnboardingState {
  ...
  storeDetails?: OnboardingStoreDetails;
...
export async function syncOnboardingData(
  ...
  try {
      ...
      if (state.finance?.bankName && state.finance?.accountNumber) {
        ...
        await tx.bankBeneficiary.create({
          data: {
            ...
            accountName: bankInfo.accountName || "Bank Account",
    ...
    logger.error("[Sync] Failed to sync onboarding data:", {
