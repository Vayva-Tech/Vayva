...
export const GET = withVayvaAPI(
  ...
  async (req: NextRequest, { storeId, correlationId }: APIContext) => {
    ...
    try {
      ...
      const [
        ...
        prisma.legalCase.findMany({
          ...
          include: {
            ...
            dockets: { select: { id: true, filingDate: true } },
          },
        }),
        
        // Comparison period data
        comparePeriod ? prisma.legalCase.findMany({
          where: {
            storeId,
        ...
        comparePeriod ? prisma.legalClient.count({
          where: {
            storeId,
            createdAt: { gte: comparisonStartDate, lt: comparisonEndDate },
          },
      ...
      const caseResolutionRate = currentCases.length > 0
        ? (currentCases.filter(c => c.status === "closed" || c.status === "settled").length / currentCases.length) * 100
        : 0;
      
      const averageCaseDuration = currentCases
        .filter(c => c.filingDate && (c.status === "closed" || c.status === "settled"))
        .map(c => (new Date().getTime() - new Date(c.filingDate!).getTime()) / (1000 * 60 * 60 * 24))
        .reduce((sum, days) => sum + days, 0) / currentCases.length || 0;
      ...
      const analyticsData = {
        overview: {
          cases: {
            ...
            averageDuration: Math.round(averageCaseDuration),
          },
          clients: {
      ...
      logger.error("[LEGAL_REPORTS_GET]", { error, storeId });
      return NextResponse.json(
