    ...
    try {
      ...
      const caseWithDetails = {
        ...
        metrics: {
          ...
          completedMatters: caseItem.matters.filter((m) => m.status === "completed")
            .length,
          matterCompletionRate:
            caseItem.matters.length > 0
              ? (caseItem.matters.filter((m) => m.status === "completed").length /
                  caseItem.matters.length) *
                100
              : 0,
          daysOpen: caseItem.filingDate
            ? Math.floor(
                (Date.now() - new Date(caseItem.filingDate).getTime()) /
                  (1000 * 60 * 60 * 24),
              )
            : 0,
          progress:
      ...
      logger.error("[LEGAL_CASE_GET]", { error, caseId: caseIdForLog });
      return NextResponse.json(
