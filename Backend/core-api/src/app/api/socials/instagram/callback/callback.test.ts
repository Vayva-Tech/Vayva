...
describe("GET /api/socials/instagram/callback", () => {
  ...
  it("stores encrypted page token and redirects to connected", async () => {
    ...
    process.env.META_IG_REDIRECT_URI =
      "https://dash.test/api/socials/instagram/callback";
    ...
    fetchMock
      ...
      .mockResolvedValueOnce({
        ok: true,
        text: async () => JSON.stringify({ access_token: "short" }),
      })
      // long-lived token exchange
      .mockResolvedValueOnce({
        ok: true,
        text: async () => JSON.stringify({ access_token: "long" }),
      })
    ...
    expect(
      settings.instagram as Record<string, unknown> | undefined,
    ).toMatchObject({
      connected: true,
      provider: "meta",
      pageId: "page_1",
    ...
    expect(res.status).toBe(307);
    expect(res.headers.get("location")).toBe(
      "https://dash.test/dashboard/socials?ig=connected",
    );
  });
});
