    ...
    try {
      ...
      if (!code) {
        return NextResponse.redirect(
          buildRedirectUrl({ ig: "error", reason: "missing_code" }),
        );
      }

      if (!state || !expectedState || state !== expectedState) {
        return NextResponse.redirect(
          buildRedirectUrl({ ig: "error", reason: "invalid_state" }),
        );
      }
      ...
      if (!appId || !appSecret || !redirectUri) {
        return NextResponse.redirect(
          buildRedirectUrl({ ig: "error", reason: "missing_config" }),
        );
      }
      ...
      if (!tokenRes.ok) {
        return NextResponse.redirect(
          buildRedirectUrl({ ig: "error", reason: "token_exchange_failed" }),
        );
      }
      ...
      if (!accountsRes.ok) {
        return NextResponse.redirect(
          buildRedirectUrl({ ig: "error", reason: "fetch_pages_failed" }),
        );
      }
      ...
      if (
        ...
        return NextResponse.redirect(
          buildRedirectUrl({ ig: "error", reason: "no_ig_business_account" }),
        );
      }
      ...
      if (!store) {
        return NextResponse.redirect(
          buildRedirectUrl({ ig: "error", reason: "store_not_found" }),
        );
      }
      ...
      logger.error("[INSTAGRAM_CALLBACK_GET]", error, { storeId });
      return NextResponse.redirect(
        buildRedirectUrl({ ig: "error", reason: "unexpected" }),
      );
    }
  },
);
