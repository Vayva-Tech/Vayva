...
function nextRetryAfter(attempt: number): Date {
  const now = new Date();
  const minutes =
    attempt <= 1 ? 2 : attempt === 2 ? 5 : attempt === 3 ? 15 : 60;
...
async function verifyCname(domain: string, expected: string): Promise<boolean> {
  try {
    ...
    return records.some(
      (r) => r.replace(/\.$/, "").toLowerCase() === normalized,
    );
  } catch (error) {
    void error;
  }
    ...
    try {
      ...
      if (!id)
        return NextResponse.json(
          { error: "Missing id", correlationId },
      ...
      if (!mapping)
        return NextResponse.json(
          { error: "Not found", correlationId },
          { status: 404, headers: { "Cache-Control": "no-store" } },
        );

      if (String(mapping.status).toUpperCase() === "VERIFIED") {
        return NextResponse.json(
          { domain: mapping, correlationId },
      ...
      const expectedTxt = String(
        provider.expectedTxt ||
          `vayva-verification=${mapping.verificationToken}`,
      );
      const expectedCname = provider.expectedCname
        ? String(provider.expectedCname)
        : null;
      ...
      try {
        ok = await Promise.race([
          ...
          expectedCname
            ? verifyCname(mapping.domain, expectedCname)
      ...
      if (ok) {
        ...
        return NextResponse.json(
          { domain: updated, correlationId },
          { headers: { "Cache-Control": "no-store" } },
        );
      }

      const failureReason = expectedCname
        ? "DNS record not found (TXT/CNAME)"
