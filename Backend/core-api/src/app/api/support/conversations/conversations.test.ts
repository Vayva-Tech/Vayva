...
describe("GET /api/support/conversations", () => {
  ...
  it("adds a contact.channel filter when channel=instagram", async () => {
    const req = new Request(
      "https://dash.test/api/support/conversations?channel=instagram",
    );

    const res = await GET(req, { storeId: "store_1" } as Parameters<
      typeof GET
    >[1]);
  ...
  it("adds a contact.channel filter when channel=whatsapp", async () => {
    const req = new Request(
      "https://dash.test/api/support/conversations?channel=whatsapp",
    );
