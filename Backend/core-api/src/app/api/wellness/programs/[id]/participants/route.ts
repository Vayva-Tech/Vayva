...
export async function GET(
  ...
  try {
    ...
    const completedEnrollments = program.enrollments.filter(e => e.status === "completed").length;
    
    const averageProgress = activeEnrollments > 0 
      ? Math.round(program.enrollments.filter(e => e.status === "active")
        .reduce((sum, e) => sum + (e.progress || 0), 0) / activeEnrollments * 100) / 100
      : 0;
    ...
    logger.error("[WELLNESS_PROGRAM_PARTICIPANTS_GET]", { error, programId: params.id });
    return NextResponse.json(
