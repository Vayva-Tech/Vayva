...
const EnrollmentSchema = z.object({
  ...
  paymentMethod: z
    .enum(["credit_card", "debit_card", "cash", "check"])
    .optional(),
  ...
  async (req: NextRequest, { storeId, params, user, correlationId }: APIContext) => {
    ...
    try {
      ...
      const existingEnrollment =
        await prisma.wellnessProgramEnrollment.findFirst({
          where: { programId: id, clientId, storeId },
        });
      ...
      logger.error("[WELLNESS_PROGRAM_ENROLLMENT_POST]", {
        error,
