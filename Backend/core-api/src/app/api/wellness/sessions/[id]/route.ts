...
export const GET = withVayvaAPI(
  ...
  async (_req: NextRequest, { storeId, params, correlationId }: APIContext) => {
    ...
    try {
      ...
      const confirmedAttendance = session.attendance.filter(
        (a) => a.status === "attended",
      ).length;
      const noShowCount = session.attendance.filter(
        (a) => a.status === "no_show",
      ).length;
      ...
      const sessionWithMetrics = {
        ...
        timing: {
          ...
          timeUntilStart: Math.ceil(
            (session.startTime.getTime() - Date.now()) / (1000 * 60),
          ),
          isUpcoming: session.startTime > new Date(),
          isInProgress:
            session.startTime <= new Date() && session.endTime >= new Date(),
      ...
      logger.error("[WELLNESS_SESSION_GET]", { error, sessionId: sessionIdForLog });
      return NextResponse.json(
