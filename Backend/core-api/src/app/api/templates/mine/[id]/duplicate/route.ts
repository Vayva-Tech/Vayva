...
export const POST = withVayvaAPI(
  ...
  async (req: NextRequest, { storeId, correlationId, params }) => {
    try {
      ...
      if (!id) {
        return NextResponse.json(
          {
            error: "Template project ID is required",
      ...
      while (
        await prisma.templateProject.findUnique({
          where: { storeId_name: { storeId, name: copyName } },
        })
      ) {
      ...
      return NextResponse.json(
        {
          error: "Failed to duplicate template project",
          requestId: correlationId,
        },
