...
export async function GET(
  ...
  try {
    ...
    const completedItems = performanceData.filter(item => item.status === "completed").length;
    const avgCompletionRate = totalItems > 0 ? (completedItems / totalItems) * 100 : 0;
    
    // Calculate average prep times
    const completedItemsWithTimes = performanceData.filter(
      item => item.status === "completed" && item.startTime && item.completionTime
    );
    
    const avgActualPrepTime = completedItemsWithTimes.length > 0
      ? completedItemsWithTimes.reduce((sum, item) => {
          const actualTime = (item.completionTime!.getTime() - item.startTime!.getTime()) / 60000;
          return sum + actualTime;
        }, 0) / completedItemsWithTimes.length
      : 0;

    const avgEstimatedPrepTime = completedItemsWithTimes.length > 0
      ? completedItemsWithTimes.reduce((sum, item) => sum + item.prepTime, 0) / completedItemsWithTimes.length
      : 0;

    // Priority performance
    const priorityStats = performanceData.reduce((acc, item) => {
    ...
    const performanceMetrics = {
      ...
      efficiency: avgEstimatedPrepTime > 0 
        ? Math.round((avgEstimatedPrepTime / avgActualPrepTime) * 100)
        : 100,
      priorityPerformance: Object.entries(priorityStats).reduce((acc, [priority, stats]) => {
        acc[priority] = {
          total: stats.total,
    ...
    logger.error("[STATION_PERFORMANCE_GET]", { error, stationId: params.id });
    return NextResponse.json(
