...
export async function GET(
  ...
  try {
    ...
    logger.error("[GROCERY_SUPPLIER_GET]", { error, supplierId: params.id });
    return NextResponse.json(
...
export async function PATCH(
  ...
  try {
    ...
    logger.error("[GROCERY_SUPPLIER_UPDATE]", { error, supplierId: params.id });
    return NextResponse.json(
...
export async function DELETE(
  ...
  try {
    ...
    logger.error("[GROCERY_SUPPLIER_DELETE]", { error, supplierId: params.id });
    return NextResponse.json(
...
async function getSupplierPerformance(supplierId: string, storeId: string) {
  ...
  const onTimeDeliveries = orders.filter(o => 
    o.deliveryDate && o.expectedDelivery && o.deliveryDate <= o.expectedDelivery
  ).length;
  const deliveryRate = orders.length > 0 ? (onTimeDeliveries / orders.length) * 100 : 0;
  ...
  return {
    totalProducts,
    lowStockRate: totalProducts > 0 ? (lowStockProducts / totalProducts) * 100 : 0,
    averageProductRating,
    totalRevenue,
    recentOrders: orders.length,
    onTimeDeliveryRate: deliveryRate,
    averageReviewRating,
    reliabilityScore: (deliveryRate * 0.4) + (averageProductRating * 0.3) + ((100 - (lowStockProducts/totalProducts)*100) * 0.3),
  };
}