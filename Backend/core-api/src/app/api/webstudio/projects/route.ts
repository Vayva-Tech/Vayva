...
export const POST = withVayvaAPI(
  ...
  async (req, { storeId, correlationId }) => {
    try {
      if (!WEBSTUDIO_TOKEN) {
        logger.error("WEBSTUDIO_API_TOKEN not configured", {
          requestId: correlationId,
          storeId,
        });
        return NextResponse.json(
          {
            error: "Webstudio integration is not configured",
            requestId: correlationId,
          },
      ...
      const templateProjectId = getString(body.templateProjectId);
      ...
      const projectConfig = getObject(project.config);
      ...
      return NextResponse.json(
        {
          error: "Failed to create Webstudio project",
          requestId: correlationId,
        },
