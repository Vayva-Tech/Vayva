...
export const GET = withVayvaAPI(
  ...
  async (_req: NextRequest, { storeId, params, correlationId }: APIContext) => {
    ...
    try {
      ...
      const mockOpportunities = [
        {
          ...
          matchScore:
            volunteerSkills.includes("Writing") &&
            volunteerSkills.includes("Research")
              ? 100
              : 50,
        ...
        },
        {
          ...
          matchScore: volunteerSkills.some((skill: string) =>
            ["Social Media", "Content Creation"].includes(skill),
          )
            ? 75
            : 25,
      ...
      return NextResponse.json(
        {
          data: {
            ...
            opportunities: {
              ...
              highMatchCount: mockOpportunities.filter((o) => o.matchScore >= 75)
                .length,
            },
      ...
      logger.error("[NONPROFIT_VOLUNTEER_SCHEDULE_GET]", {
        error,
...
function generateScheduleRecommendations(
  ...
  if (availableDays.length === 0) {
    recommendations.push(
      "No availability specified - please update your availability preferences",
    );
  } else if (availableDays.length <= 2) {
    recommendations.push(
      `Limited availability (${availableDays.join(", ")}) - consider expanding available days`,
    );
  }

  if (skills.length === 0) {
    recommendations.push(
      "No skills listed - add your skills to receive better opportunity matching",
    );
  }
  ...
  if (upcomingCommitments === 0) {
    ...
    recommendations.push(
      "High commitment level - ensure adequate rest between activities",
    );
  }

  if (availableDays.includes("Saturday") || availableDays.includes("Sunday")) {
    recommendations.push(
      "Weekend availability is valuable for community events",
    );
  }

  return recommendations.length > 0
    ? recommendations
    : ["Availability looks good - keep up the great work!"];
}
