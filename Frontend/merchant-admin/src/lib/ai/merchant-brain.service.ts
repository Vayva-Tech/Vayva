...
export class MerchantBrainService {
    ...
    static async searchCatalog(storeId: string, query: string) {
        try {
            ...
            for (const row of stockAgg as Array<{ productId: string; _sum: { available: number | null } }>) {
    ...
    static async getStoreFulfillmentPolicy(storeId: string) {
        try {
            ...
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const normalizedPickupLocations = pickupLocations.map((l: { id: string; name: string; address: string; city: string; state: string; isPickupPoint?: boolean; isDefault?: boolean }) => ({
                id: l.id,
                name: l.name,
                address: l.address,
                city: l.city,
    ...
    static async retrieveContext(storeId: string, query: any, limit = 3) {
        try {
            ...
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            return embeddings.map((e: { content: string; sourceType: string; sourceId: string; metadata: unknown }) => ({
                content: e.content,
    ...
    static async getDeliveryQuote(storeId: string, location: any) {
        try {
            ...
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const matchedZone = zones.find((z: { name?: string; states?: string[]; cities?: string[]; feeAmount: unknown; etaMinDays: number; etaMaxDays: number }) => z.name?.toLowerCase().includes(location.toLowerCase()) ||
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                z.states?.some((s: string) => location.toLowerCase().includes(s.toLowerCase())) ||
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                z.cities?.some((c: string) => location.toLowerCase().includes(c.toLowerCase())));
            if (matchedZone) {
                return {
                    location,
    ...
    static async getActivePromotions(storeId: string) {
        try {
            ...
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            return promos.map((p: { id: string; name: string; type: string; valueAmount?: unknown; valuePercent?: unknown; requiresCoupon?: boolean }) => ({
                id: p.id,
                name: p.name,
                type: p.type,
