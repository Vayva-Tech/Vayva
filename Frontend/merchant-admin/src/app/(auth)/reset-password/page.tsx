  ...
  return (
    ...
    >
        ...
        <div className="space-y-2">
          ...
          <div className="relative">
            ...
            <Button
              ...
              className="absolute right-3 top-1/2 -translate-y-1/2 text-text-tertiary hover:text-black h-8 w-8"
          ...
          <div className="mt-2 p-3 bg-muted/50 rounded-lg">
            <p className="text-xs text-text-tertiary mb-2">Password must contain:</p>
            <ul className="space-y-1">
              {requirements.map((req) => {
                const isMet = password ? req.test(password) : false;
                return (
                  <li key={req.label} className="text-xs flex items-center gap-1.5">
                    <span className={`w-3.5 h-3.5 rounded-full flex items-center justify-center text-[10px] ${isMet ? "bg-status-success text-white" : "bg-white/40 text-text-tertiary"}`}>
                      {isMet ? "✓" : "•"}
                    </span>
                    <span className={isMet ? "text-status-success" : "text-text-tertiary"}>
                      {req.label}
                    </span>
                  </li>
                );
      ...
      <div className="mt-6 text-center">
        <Link
          href="/signin"
          className="text-sm text-text-primary hover:text-black font-medium transition-colors inline-flex items-center gap-1"
        >
      ...
      fallback={
        ...
        >
          <div className="flex justify-center py-12">
            <Icon
              name="Loader2"
              className="w-8 h-8 animate-spin text-text-tertiary"
            />
          </div>
        </SplitAuthLayout>
      }
