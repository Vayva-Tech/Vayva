...
const VerifyContent = () => {
  ...
  const handleVerify = async (otpValue: string) => {
    ...
    try {
      const response = await AuthService.verifyOTP(email, otpValue, method);
  ...
  const handleResend = async () => {
    ...
    try {
      await AuthService.resendCode({ email, method });
      setResendTimer(30);
      setCanResend(false);
      setError(null);
    } catch (err: unknown) {
      ...
      <div className="text-center mb-6">
        {canResend ? (
          <Button
            ...
            className="text-sm text-text-primary hover:text-black font-medium pl-0 pr-0"
          >
            {resendLoading ? (
              <>
      ...
      <div className="mt-6 text-center space-y-2">
        <p className="text-xs text-text-tertiary">
          ...
          <Button
            ...
            className="text-text-primary hover:text-black disabled:opacity-50 disabled:cursor-not-allowed font-medium pl-0 pr-0 h-auto"
          >
        ...
        <p className="text-xs text-text-tertiary">
          ...
          <Link
            ...
            className="text-text-primary hover:text-black font-medium underline"
          >
            Go back to update
          </Link>
        </p>
