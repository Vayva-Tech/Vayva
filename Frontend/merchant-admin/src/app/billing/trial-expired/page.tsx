    ...
    <div className="min-h-screen bg-white/40 flex flex-col items-center justify-center p-6">
      <div className="max-w-4xl w-full space-y-12 text-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-2xl mx-auto">
          <Card className="p-6 bg-background border-2 border-border/40 hover:border-black transition-all">
            <div className="flex flex-col items-center gap-2">
              <span className="text-3xl font-bold text-black">
                {isLoading ? "..." : stats.products}
              </span>
              <span className="text-sm font-bold uppercase tracking-wider text-text-tertiary">
                Products Online
              </span>
            </div>
          </Card>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-12">
          {PLANS.filter((p: any) => p.key !== "STARTER").map((plan: { key: string; name: string; tagline?: string; monthlyAmount: number; bullets?: string[] }) => (
            <Card
              key={plan.key}
              className={`p-8 bg-background flex flex-col relative ${plan.key === "PRO" ? "border-2 border-black scale-105 z-10" : "border border-border"}`}
            >
              {plan.key === "PRO" && (
              ...
              <div className="mb-6">
                ...
                <p className="text-text-tertiary text-sm h-10">
                  {plan.tagline}
                </p>
              </div>
              ...
              <div className="flex items-baseline gap-1 mb-8">
                ...
                <span className="text-text-tertiary font-medium">/month</span>
              </div>
              ...
              <Button
                className={`w-full h-14 text-lg font-bold rounded-xl transition-transform active:scale-95 ${plan.key === "PRO" ? "bg-black text-white hover:bg-text-primary shadow-xl" : "bg-background text-black border-2 border-border hover:border-black"}`}
