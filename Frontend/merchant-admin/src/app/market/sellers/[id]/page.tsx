    ...
    return (
      <MarketShell>
        <div className="animate-pulse">
          <div className="h-48 bg-muted mb-8" />
          <div className="max-w-6xl mx-auto px-4">
            <div className="h-8 bg-muted rounded w-1/3 mb-4" />
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[...Array(8)].map((_, i) => <div key={i} className="h-64 bg-muted rounded" />)}
            </div>
          </div>
        </div>
      </MarketShell>
  ...
  return (
    <MarketShell>
      ...
      <div className="max-w-6xl mx-auto px-4 -mt-16 relative z-10">
        {/* Seller Header */}
        <div className="bg-surface-1 border rounded-xl p-6 mb-8">
            ...
            <div className="w-24 h-24 rounded-xl bg-muted overflow-hidden flex-shrink-0">
              {seller.logo ? (
                ...
                <div className="flex items-center justify-center h-full">
                  <Icon name="Store" size={40} className="text-muted-foreground" />
                </div>
              )}
            </div>
        ...
        {categories.length > 1 && (
          <div className="flex gap-2 overflow-x-auto mb-6">
            <button
              ...
              className={`px-4 py-2 rounded-full text-sm whitespace-nowrap ${!activeCategory ? "bg-primary text-white" : "bg-muted"}`}
            >
              All Products
            </button>
            {categories.map((cat) => (
              <button
                ...
                className={`px-4 py-2 rounded-full text-sm whitespace-nowrap capitalize ${activeCategory === cat ? "bg-primary text-white" : "bg-muted"}`}
              >
                {cat}
              </button>
            ))}
