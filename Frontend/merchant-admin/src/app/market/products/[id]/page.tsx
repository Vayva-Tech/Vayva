    ...
    <MarketShell>
      <div className="max-w-6xl mx-auto py-8 px-4">
        ...
        <div className="flex items-center gap-2 text-sm text-muted-foreground mb-6">
          ...
          <Link href={`/market/categories/${product.category}`} className="hover:text-primary capitalize">{product.category}</Link>
          <Icon name="ChevronRight" size={16} />
        ...
        <div className="grid md:grid-cols-2 gap-8">
              ...
              <div className="flex gap-2">
                {product.images.map((img, idx) => (
                  <button
                    ...
                    className={`w-20 h-20 rounded-lg overflow-hidden border-2 ${selectedImage === idx ? "border-primary" : "border-transparent"}`}
                  >
          ...
          <div>
            ...
            <Link href={`/market/sellers/${product.seller.id}`} className="text-sm text-muted-foreground hover:text-primary mb-4 inline-block">
              ...
              <div className="flex items-center gap-1">
                {[1, 2, 3, 4, 5].map((star) => (
                  <Icon
                    ...
                    name="Star"
                    size={18}
                    className={star <= Math.round(product.rating) ? "text-yellow-500 fill-yellow-500" : "text-muted-foreground"}
                  />
                ))}
              </div>
            ...
            <div className="flex items-baseline gap-3 mb-6">
              ...
              {product.compareAtPrice && (
                <span className="text-xl text-muted-foreground line-through">₦{product.compareAtPrice.toLocaleString()}</span>
              )}
            </div>
            ...
            <div className="flex items-center gap-4 mb-6">
              ...
              <span className="text-sm text-muted-foreground">{product.inStock ? `${product.quantity} available` : "Out of stock"}</span>
            </div>
