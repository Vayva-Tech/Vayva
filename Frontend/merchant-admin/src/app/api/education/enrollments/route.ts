 ...
 */
export const GET = withVayvaAPI(PERMISSIONS.PRODUCTS_VIEW, async (req: NextRequest, { storeId, user }: { storeId: string; user: { id: string } }) => {
    try {
        ...
        logger.error("[ENROLLMENTS_GET]", error instanceof Error ? error.message : String(error));
