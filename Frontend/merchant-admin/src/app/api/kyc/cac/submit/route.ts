...
export const POST = withVayvaAPI(PERMISSIONS.KYC_MANAGE, async (req: NextRequest, { storeId, user, correlationId }: { storeId: string; user: { id: string }; correlationId: string }) => {
    try {
        ...
        logger.error("[KYC_CAC_SUBMIT]", { error: message, stack, requestId: correlationId, storeId });
        return NextResponse.json(
