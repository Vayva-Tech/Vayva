...
export const POST = withVayvaAPI(PERMISSIONS.FINANCE_VIEW, async (req: any) => {
    try {
        const body = await req.json().catch(() => ({}));
        const parsed = parseMonthYear({ month: body?.month, year: body?.year });
        if (!parsed) {
            return NextResponse.json({ error: "Invalid month/year" }, { status: 400 });
        }
        const { monthIndex, year } = parsed;
        const url = `/api/finance/statements/generate?month=${encodeURIComponent(String(body?.month))}&year=${encodeURIComponent(String(year))}`;
        return NextResponse.json({ url, monthIndex, year }, {
