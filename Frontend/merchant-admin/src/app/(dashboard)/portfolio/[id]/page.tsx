...
export default function ProjectEditorPage() {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-64px)] overflow-hidden">
      {/* Toolbar */}
      <div className="h-16 border-b bg-background/70 backdrop-blur-xl flex items-center justify-between px-6 shrink-0 z-10">
        <div className="flex items-center gap-4">
          <BackButton href="/dashboard/portfolio" />
          <div className="h-6 w-px bg-border" />
          <div>
            <h1 className="font-semibold text-text-primary truncate max-w-xs">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 bg-background/30 overflow-y-auto p-8">
          <div className="max-w-4xl mx-auto space-y-8">
            {/* Empty State / Upload Zone */}
            <div className="border-2 border-dashed border-border rounded-xl p-12 text-center bg-background/70 backdrop-blur-xl hover:bg-background/30 transition-colors cursor-pointer relative">
              <Input title="Upload project images"
                type="file"
                multiple
                accept="image/*"
              ...
              />
              <Upload className="mx-auto h-10 w-10 text-text-tertiary mb-3" />
            ...
            {images.length > 0 && (
              <div className="columns-2 md:columns-3 gap-4 space-y-4">
                {images.map((img, idx) => (
                  <div
                    key={img.id || idx}
                    className="break-inside-avoid relative group rounded-lg overflow-hidden bg-background/70 backdrop-blur-xl shadow-sm hover:shadow-md transition-all"
                  >
                    <img
                      src={img.url}
                      alt={img.caption}
        ...
        {/* Sidebar: Settings */}
        <div className="w-80 bg-background/70 backdrop-blur-xl border-l p-6 overflow-y-auto shrink-0 flex flex-col gap-6">
          <div>
          ...
          <div>
            ...
            <div className="flex items-start justify-between mb-4">
              <div className="space-y-0.5">
                ...
                <p className="text-xs text-text-tertiary">
                  Enable private access & commenting.
                </p>
              </div>
            ...
            {clientMode && (
              <div className="bg-amber-50 rounded-lg p-4 border border-amber-100 space-y-3 animate-in fade-in slide-in-from-top-2">
                <div>
                  <Label htmlFor="pwd" className="text-amber-900">
                    Access Password
                  </Label>
                  <Input
                    id="pwd"
