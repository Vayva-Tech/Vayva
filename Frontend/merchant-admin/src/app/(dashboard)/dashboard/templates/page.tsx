...
function CommerceBlocksPanel({ enabledExtensions }: { enabledExtensions: string[] }) {
  ...
  return (
    <div className="bg-white/40 border border-border/40 rounded-2xl p-5 space-y-4">
      ...
      {enabledExtensions.length > 0 && extensionBlocks.length > 0 && (
        <div className="space-y-2">
          <p className="text-[9px] font-black uppercase tracking-widest text-text-tertiary px-1">
          ...
          <div className="flex flex-wrap gap-2">
            {extensionBlocks.slice(0, 6).map((block) => (
              <span
                key={block.key}
                className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-primary/5 border border-primary/10 text-[10px] font-medium"
              >
                <Icon name={(block.icon as any) || "Box"} size={12} className="text-primary/60" />
                {block.name}
              </span>
            ))}
            {extensionBlocks.length > 6 && (
              <span className="inline-flex items-center px-2.5 py-1 rounded-full bg-background/50 text-[10px] text-text-tertiary">
      ...
      <div className="space-y-2">
        ...
        <div className="flex flex-wrap gap-2">
          {coreBlocks.slice(0, 4).map((block) => (
            <span
              key={block.key}
              className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-white/50 border border-border/30 text-[10px] text-text-secondary"
            >
              <Icon name={(block.icon as any) || "Box"} size={12} className="text-text-tertiary" />
              {block.name}
            </span>
          ))}
        </div>
      ...
      <Tabs defaultValue="system" className="w-full">
        <div className="flex flex-col gap-8">
          <div className="flex flex-col md:flex-row items-center justify-between gap-6">
            <TabsList className="bg-background/30 border border-border/40 rounded-[20px] p-1.5 h-auto shrink-0">
              <TabsTrigger
                value="system"
                className="rounded-[14px] data-[state=active]:bg-background/70 backdrop-blur-xl data-[state=active]:shadow-md px-6 py-2.5 text-sm font-bold transition-all"
              >
                Template Library
              </TabsTrigger>
              <TabsTrigger
              ...
              {loadingMine ? (
                ...
                >
                  <div className="mt-16 w-full max-w-3xl mx-auto">
                    <p className="text-[10px] font-black uppercase tracking-widest text-text-tertiary mb-6 text-center">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                      {systemTemplates.slice(0, 3).map((t) => (
                        <Button
                          ...
                          className="rounded-[24px] border border-border/40 bg-background/60 p-5 text-left hover:bg-white/80 dark:hover:bg-zinc-900 hover:shadow-xl hover:border-primary/20 transition-all duration-300 group/item active:scale-[0.98] h-auto block"
                        >
                          <div className="w-12 h-12 rounded-2xl bg-primary/5 flex items-center justify-center mb-4 group-hover/item:bg-primary/10 transition-colors">
                            <Icon
                              name="LayoutTemplate"
                              size={20}
                          ...
                          <p className="text-sm font-bold text-text-primary tracking-tight truncate">
                            {t.name}
                          </p>
                          <p className="text-[10px] font-black uppercase tracking-tight text-text-tertiary mt-1">
                            {t.category}
                          </p>
                        </Button>
                      ))}
