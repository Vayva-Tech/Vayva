  ...
  if (!site) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] gap-4">
        <Icon name="AlertCircle" size={48} className="text-text-tertiary" />
        ...
        <div className="space-y-4">
          {site.recentActivity && site.recentActivity?.length > 0 ? (
            site.recentActivity?.slice(0, 3).map((activity, i) => (
              <div
                key={i}
                className="flex items-center justify-between gap-4 p-4 rounded-2xl bg-background/20 border border-border/20"
              >
