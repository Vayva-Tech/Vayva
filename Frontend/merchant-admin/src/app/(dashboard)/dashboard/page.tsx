  ...
  if (!merchant)
    return (
      <div className="space-y-6">
        <div className="h-8 w-48 bg-background/30 rounded-xl animate-pulse mb-2" />
        <div className="h-4 w-64 bg-background/30 rounded-xl animate-pulse mb-8" />
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          {[1, 2, 3, 4].map((i) => (
            <div
              key={i}
              className="h-32 bg-background/30 rounded-2xl animate-pulse"
            />
          ))}
        </div>
      </div>
