      ...
      <aside className="w-[300px] border-r border-white/5 flex flex-col fixed inset-y-0 bg-[#0b141a]/95 backdrop-blur-xl z-50">
              ...
              <Link key={item.path} href={item.path}>
                <div
                  className={`flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-colors
                                    ${isActive ? "bg-background/10 text-white" : "text-text-secondary hover:text-white hover:bg-background/5"}
                                `}
                >
                  <Icon
                    name={item.icon}
        ...
        <div className="p-4 border-t border-white/5">
          <div className="flex items-center gap-3 px-4 py-3">
            <div className="w-8 h-8 rounded-full bg-background/10 flex items-center justify-center text-xs font-bold">
              JD
            </div>
        ...
        <header className="h-[72px] border-b border-white/5 bg-[#142210]/90 backdrop-blur sticky top-0 z-40 px-8 flex items-center justify-between gap-8">
          ...
          <div className="flex-1 max-w-2xl relative">
            <Icon
              ...
              size={18}
            />
            <Input className="w-full bg-background/5 border border-white/10 rounded-full pl-10 pr-4 py-2 text-sm text-white focus:outline-none focus:border-white/20 placeholder:text-text-secondary transition-all"
              type="search"
              aria-label="Search merchant, order ID, or transaction reference"
              placeholder="Search merchant, order ID, transaction reference..."
            />
            <div className="absolute right-2 top-1.5 flex gap-1">
              <kbd className="px-1.5 py-0.5 rounded bg-background/5 border border-white/10 text-[10px] font-mono text-text-secondary">
                ⌘
              </kbd>
              <kbd className="px-1.5 py-0.5 rounded bg-background/5 border border-white/10 text-[10px] font-mono text-text-secondary">
                K
