  ...
  return (
    <div className="fixed inset-0 z-50">
      ...
      <div className="fixed left-1/2 top-[20%] -translate-x-1/2 w-full max-w-[640px] px-4">
        <Command
          className="bg-background/95 backdrop-blur-xl rounded-2xl border border-border shadow-2xl overflow-hidden"
          loop
        >
          <div className="flex items-center gap-3 px-4 py-4 border-b border-border">
            ...
            <Command.Input
              ...
              autoFocus
            />
            <kbd className="px-2 py-1 text-xs font-mono bg-background/50 border border-border rounded text-text-tertiary">
              ESC
            </kbd>
          </div>
            ...
            {Object.entries(filteredGrouped).map(([section, items]) =>
              items.length > 0 ? (
                <Command.Group
                  ...
                  className="px-2 py-2"
                >
                  <div className="text-xs font-medium text-text-tertiary uppercase tracking-wider px-2 py-1.5 mb-1">
                    {section}
                  </div>
                  {items.map((item: CommandItem) => (
                    <Command.Item
                      ...
                      className="flex items-center gap-3 px-3 py-2.5 rounded-xl cursor-pointer transition-colors hover:bg-background/50 data-[selected=true]:bg-indigo-50 data-[selected=true]:text-indigo-700"
                    >
                      {item.icon && (
                      ...
                      {item.shortcut && (
                        <kbd className="px-2 py-0.5 text-xs font-mono bg-background/50 border border-border rounded text-text-tertiary">
                          {item.shortcut}
                        </kbd>
                      )}
                    </Command.Item>
          ...
          <div className="flex items-center justify-between px-4 py-3 border-t border-border bg-background/30">
            <div className="flex items-center gap-4 text-xs text-text-tertiary">
              <span className="flex items-center gap-1">
                <kbd className="px-1.5 py-0.5 font-mono bg-background/50 border border-border rounded">
                  ↑↓
                </kbd>
                to navigate
              </span>
