  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        ...
        <h3 className="font-semibold text-text-primary">Color Options</h3>
      </div>
      ...
      {value.length > 0 && (
        <div className="flex flex-wrap gap-2">
          {value.map((swatch) => (
            <div
              key={swatch.id}
              className="group flex items-center gap-2 px-2 py-1.5 bg-background/50 rounded-lg border border-border/40"
            >
              {swatch.imageUrl ? (
                <img
                  src={swatch.imageUrl}
                ...
                <div
                  className="w-6 h-6 rounded border border-border/40 shadow-sm"
      ...
      <div className="space-y-2">
        <Label className="text-xs text-text-tertiary">Quick Add Presets</Label>
        <div className="flex flex-wrap gap-2">
          {PRESET_COLORS.map((color) => (
            <Button
              ...
              className="flex items-center gap-1.5 px-2 py-1 rounded border border-border/40 hover:bg-background/50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              title={color.name}
            >
              <div
