...
export function TodosAlerts() {
  ...
  const getAlertColor = (type: string) => {
    switch (type) {
      ...
      default:
        return "bg-surface-2/50 border-border text-text-primary";
    }
  };
  ...
  if (loading) {
    return (
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <Card className="p-6">
          <div className="h-6 w-32 bg-surface-2/50 rounded-xl animate-pulse mb-4" />
          <div className="space-y-3">
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className="h-20 bg-surface-2/50 rounded-xl animate-pulse"
              />
            ))}
          </div>
        </Card>
        <Card className="p-6">
          ...
          <div className="space-y-3">
            {[1, 2].map((i) => (
              <div
                key={i}
                className="h-16 bg-surface-2/50 rounded-xl animate-pulse"
              />
            ))}
          </div>
        </Card>
          ...
          <div className="space-y-3" role="list" aria-label="Todo items">
            {todos.map((todo) => (
              <Link
                ...
                key={todo.id}
                role="listitem"
                className="p-4 border border-border rounded-xl hover:bg-surface-2/50 transition-colors"
              >
                <div className="flex items-start gap-3">
                  <div className="p-2 bg-surface-2/50 rounded-xl shrink-0">
                    <Icon
                      name={todo.icon}
            ...
            {alerts.map((alert) => (
              ...
              >
                <button
                  ...
                  className="absolute top-2 right-2 p-1 rounded-lg hover:bg-surface-2/50 opacity-0 group-hover:opacity-100 transition-opacity"
