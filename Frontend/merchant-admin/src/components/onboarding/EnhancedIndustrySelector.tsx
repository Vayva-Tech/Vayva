...
export function EnhancedIndustrySelector({
  ...
  return (
    <div className="space-y-3">
      ...
      <div 
        className="relative bg-white border-2 border-border rounded-xl p-4 cursor-pointer hover:border-vayva-green transition-colors"
      ...
      >
        <div className="flex items-center justify-between">
          ...
          <div className="i-ph-caret-down text-text-tertiary transform transition-transform" 
      ...
      {/* Dropdown */}
      {isOpen && (
        <div className="absolute z-50 w-full bg-white border border-border rounded-xl shadow-xl mt-1 max-h-96 overflow-hidden">
          {/* Search Bar */}
          <div className="p-3 border-b border-border sticky top-0 bg-white">
            <div className="relative">
              <MagnifyingGlass className="absolute left-3 top-1/2 -translate-y-1/2 text-text-tertiary w-4 h-4" />
              <input
                type="text"
                placeholder="Search industries..."
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
                className="w-full pl-10 pr-4 py-2 rounded-lg border border-border focus:border-vayva-green focus:ring-1 focus:ring-vayva-green outline-none text-sm"
                autoFocus
              />
            </div>
          </div>
          ...
          <div className="overflow-y-auto max-h-80">
            ...
            {smartRecommendations.length > 0 && !searchTerm && (
              <div className="p-3 border-b border-border bg-gradient-to-r from-vayva-green/5 to-transparent">
                <div className="flex items-center gap-2 mb-2">
                  <Sparkle className="w-4 h-4 text-vayva-green" />
                  <h4 className="font-bold text-sm text-text-primary">Smart Matches</h4>
                </div>
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div className="text-left">
                            <p className="font-medium text-text-primary text-sm group-hover:text-vayva-green">
            ...
            {Object.entries(filteredCategories).map(([category, config]) => (
              <div key={category} className="border-b border-border last:border-b-0">
                <div className="sticky top-0 bg-gray-50 px-3 py-2 border-b border-border">
                  <div className="flex items-center gap-2">
                    <config.icon className={`w-4 h-4 ${config.color}`} />
                    <h3 className="font-bold text-xs text-text-secondary uppercase tracking-wider">
                      {category}
                    </h3>
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            <p className={`font-medium ${
                              isSelected ? 'text-vayva-green' : 'text-text-primary'
                            }`}>
                              {industry.displayName}
                              {isSelected && (
            ...
            {Object.keys(filteredCategories).length === 0 && (
              <div className="p-8 text-center">
                ...
                <p className="text-text-secondary mb-2">No industries found</p>
