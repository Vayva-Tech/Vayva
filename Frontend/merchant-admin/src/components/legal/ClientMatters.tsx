...
export function ClientMatters({ data }: ClientMattersProps) {
  ...
  return (
    <Card className="p-6 border-l-4 border-cyan-700 shadow-lg backdrop-blur-sm bg-white/90">
      <div className="flex items-center gap-2 mb-4">
        <Users size={24} className="text-cyan-700" />
        <h2 className="text-xl font-bold text-text-primary">Client Matters</h2>
      </div>
          ...
          <div key={idx} className="p-3 bg-gray-50 rounded-lg border border-gray-200">
            <div className="flex items-center justify-between mb-1">
              ...
              <span className={`text-xs px-2 py-1 rounded ${
                matter.status === 'Active' ? 'bg-green-100 text-green-800' : 'bg-amber-100 text-amber-800'
              }`}>
                {matter.status}
              </span>
            </div>
            <div className="text-xs text-text-secondary">{matter.caseNumber} • {matter.practiceArea}</div>
      ...
      {(data.lowRetainerAlerts > 0 || data.overdueInvoices > 0) && (
        <div className="mt-4 pt-4 border-t border-gray-200 space-y-2">
          {data.lowRetainerAlerts > 0 && (
            <div className="text-xs text-amber-800 bg-amber-50 p-2 rounded">
