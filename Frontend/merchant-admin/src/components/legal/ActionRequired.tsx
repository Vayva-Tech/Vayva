...
export function ActionRequired({ items, onRefresh }: ActionRequiredProps) {
  ...
  return (
    <Card className="p-6 border-l-4 border-orange-600 shadow-lg backdrop-blur-sm bg-white/90">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          <CheckSquare size={24} className="text-orange-600" />
          <h2 className="text-xl font-bold text-text-primary">Action Required</h2>
        </div>
        {onRefresh && (
      ...
      {pendingItems.length === 0 ? (
        ...
        <div className="space-y-4">
              ...
              {urgentItems.map((item) => (
                ...
                >
                  ...
                  <div className="flex-1">
                    ...
                    {item.caseNumber && (
                      <div className="text-xs text-text-secondary mt-1">Case: {item.caseNumber}</div>
                    )}
          ...
          {normalItems.length > 0 && (
            <div className="space-y-2">
              <h3 className="text-sm font-semibold text-text-secondary uppercase tracking-wide">Normal Priority</h3>
