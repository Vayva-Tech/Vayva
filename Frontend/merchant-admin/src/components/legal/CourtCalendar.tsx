...
export function CourtCalendar({ data }: CourtCalendarProps) {
  ...
  return (
    <Card className="p-6 border-l-4 border-blue-700 shadow-lg backdrop-blur-sm bg-white/90">
      <div className="flex items-center gap-2 mb-4">
        <Calendar size={24} className="text-blue-700" />
        <h2 className="text-xl font-bold text-text-primary">Court Calendar</h2>
      </div>

      {/* Today's Schedule */}
      <div className="mb-4">
        <h3 className="text-sm font-semibold text-text-secondary mb-2">Today's Schedule</h3>
        {todaysAppearances.length === 0 ? (
          <p className="text-sm text-text-secondary italic">No court appearances today</p>
        ) : (
          <div className="space-y-2">
            {todaysAppearances.map((appearance) => (
              ...
              >
                ...
                <div className="text-sm text-text-primary font-medium">{appearance.title}</div>
      ...
      <div className="pt-4 border-t border-gray-200">
        <h3 className="text-sm font-semibold text-text-secondary mb-2">Upcoming Hearings</h3>
        {upcomingHearings.slice(0, 3).map((hearing) => (
          <div key={hearing.id} className="py-2 border-b border-gray-100 last:border-0">
            <div className="flex items-center justify-between">
              <span className="text-sm text-text-primary">{hearing.title}</span>
