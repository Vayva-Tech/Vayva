...
export function ConsentBanner() {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 border border-studio-border/70 bg-white/95 backdrop-blur-xl shadow-[0_20px_80px_-30px_rgba(15,23,42,0.45)] px-4 py-5 md:px-8 transition-transform duration-300">
      <div className="max-w-5xl mx-auto space-y-5">
            ...
            {!showPreferences && (
              <>
                <Button
                  ...
                  className="border-studio-border text-text-primary hover:bg-studio-gray"
                >
                  Configure
                </Button>
                <Button size="sm" onClick={handleAccept} className="bg-primary text-white hover:bg-primary-hover">
                  Accept all
                </Button>
              </>
            )}
        ...
        {showPreferences && (
          <div className="rounded-3xl border border-studio-border bg-white/90 p-4 md:p-6 shadow-[0_25px_90px_-35px_rgba(15,23,42,0.35)] space-y-5">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2">
              <div>
                <h4 className="text-base font-semibold text-text-primary">Manage cookie preferences</h4>
                <p className="text-sm text-text-secondary">
                  Toggle the non-essential categories to control how we personalize your experience.
                </p>
              </div>
              <span className="text-xs font-semibold text-text-tertiary uppercase tracking-wide">Optional</span>
            </div>
              ...
              {preferenceDetails.map(({ key, title, description, locked }) => (
                ...
                >
                  <input
                    ...
                    className="mt-1 h-4 w-4 rounded border-studio-border text-primary focus:ring-primary focus:ring-offset-0"
            ...
            <div className="flex flex-wrap justify-end gap-3">
              ...
              <Button size="sm" onClick={handleSave} className="bg-primary text-white hover:bg-primary-hover">
                Save preferences
              </Button>
            </div>
          </div>
      ...
      <Button
        ...
        className="absolute top-4 right-4 text-text-tertiary hover:text-text-secondary md:hidden h-auto w-auto p-1"
