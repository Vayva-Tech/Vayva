...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const { email, feature, industry } = body;
    if (!email?.trim() || !feature?.trim()) {
      return NextResponse.json(
    ...
    if (email.length > 254 || feature.length > 5000) {
      return NextResponse.json(
    ...
    if (process.env?.RESEND_API_KEY) {
      try {
        ...
        await resend.emails?.send({
          ...
          replyTo: email,
          subject: `[Feature Request] ${industry || "General"}: ${feature.slice(0, 50)}...`,
          html: `
            <h1>New Feature Request</h1>
            <p><strong>Industry:</strong> ${industry || "Not specified"}</p>
            <p><strong>Email:</strong> ${email}</p>
            <p><strong>Feature Request:</strong></p>
            <p style="background: #f5f5f5; padding: 16px; border-radius: 8px;">${feature.replace(/\n/g, "<br>")}</p>
            <hr>
        ...
        await resend.emails?.send({
          ...
          to: [email],
          subject: `We received your feature request${industry ? ` for ${industry}` : ""}`,
          html: `
            <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
              <h1 style="color: #16a34a;">Thank you for your suggestion!</h1>
              <p>We've received your feature request${industry ? ` for the <strong>${industry}</strong> industry` : ""}.</p>
              <div style="background: #f9fafb; padding: 20px; border-radius: 12px; margin: 20px 0;">
                <p style="margin: 0; font-size: 14px; color: #6b7280;">Your request:</p>
                <p style="margin: 10px 0 0 0; font-style: italic;">"${feature.slice(0, 200)}${feature.length > 200 ? '...' : ''}"</p>
              </div>
    ...
    await prisma.rescueIncident?.create({
      data: {
        ...
        errorMessage: `[${industry || "General"}] from ${email}: ${feature.slice(0, 100)}...`,
