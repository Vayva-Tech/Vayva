...
export default function ToolsStep() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setIsLoading(true);
        const res = await apiJson<ToolsApiResponse>("/merchant/tools");
        if (!mounted) return;
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white border border-gray-100 rounded-[32px] p-6 ">
        {isLoading ? (
          ...
          <div className="text-sm text-gray-400">No tools found.</div>
        ) : (
                ...
                <Button
                  ...
                  className={cn(
                    ...
                    isChecked
                      ...
                      : "border-gray-100 bg-white/30 hover:bg-white",
      ...
      <Button
        ...
        className="w-full h-14 bg-text-green-500 hover:bg-zinc-800 text-white rounded-2xl font-bold text-lg shadow-xl transition-all active:scale-[0.98]"
      >
        Continue
      </Button>
    </div>
