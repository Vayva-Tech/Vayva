...
export default function PaymentStep() {
  ...
  const handleContinue = () => {
    ...
    // If only bank selected, require account number
    if (selectedBankCode && !trimmedAccountNumber) {
      toast.error("Please enter your account number.");
      return;
    }
    
    // If only account number entered, allow without bank (can update later)
    if (trimmedAccountNumber && !selectedBankCode) {
      const financeData = {
        finance: {
    ...
    const financeData = {
      finance: {
        bankCode: selectedBankCode, // Optional field
        bankName: bank?.name || "",
        accountNumber: trimmedAccountNumber,
      ...
      <div className="bg-white border border-gray-100 rounded-2xl p-6  space-y-4">
        <div className="space-y-3">
          <div className="space-y-2 relative">
            ...
            <div className="relative">
              <Input
                id="bankSearch"
                type="text"
                placeholder={loadingBanks ? "Loading Nigerian banks..." : "Search bank name..."}
          ...
          <div className="space-y-2">
            ...
            {!selectedBankCode && (
              <p className="text-xs text-gray-400">Select your bank for automatic verification (optional)</p>
            )}
          </div>
