...
export function buildStepSequence(
  ...
  // Determine if we should simplify the flow by default or based on state
  const isSimplified = shouldUseSimplifiedFlow(state);

  // If not simplified, include specialized steps (B2B, nonprofit, events)
  if (!isSimplified) {
    // Check if merchant needs B2B wholesale features
    const isB2B =
      state.business?.businessType === "b2b" ||
      state.business?.enableWholesale === true ||
      state.metadata?.requiresB2B === true;

    // Check if merchant is a nonprofit organization
    const isNonprofit =
      state.business?.organizationType === "nonprofit" ||
      state.business?.businessType === "nonprofit" ||
      state.metadata?.isNonprofit === true;

    // Check if merchant needs event management
    const hasEvents =
      state.business?.hasEvents === true ||
  ...
  // Core commerce steps (either simplified or full flow)
  if (state && shouldUseSimplifiedFlow(state)) {
    // Simplified core: include a smaller set to speed up onboarding
    const CORE_CORE_SIMPLIFIED: OnboardingStepId[] = [
      "tools",
      "first_item",
      "socials",
      "finance",
    ];
    steps.push(...CORE_CORE_SIMPLIFIED);
  } else {
 ...
 */
export function shouldUseSimplifiedFlow(
  state: Partial<OnboardingState>,
): boolean {
  // Solo entrepreneurs or very small businesses
  const isSolo =
    state.business?.employeeCount === 1 ||
 ...
 */
export function getBusinessStepType(
  state: Partial<OnboardingState>,
): "business" | "simplified_business" {
...
export function shouldIncludeStep(
  ...
  // If simplified flow, skip specialized steps entirely except for non-base items that
  // are safe defaults (we'll rely on finalization) – this function gates per-step visibility.
  if (isSimplified) {
    const allowedInSimplified: OnboardingStepId[] = [
      "welcome",
      "plan_selection",
      "identity",
      "business",
    ...
    return allowedInSimplified.includes(step);
  }
