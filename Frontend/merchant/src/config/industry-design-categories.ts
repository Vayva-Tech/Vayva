...
export const DEFAULT_PRESETS: Record<DesignCategory, string> = {
...
export function getDesignCategoryForIndustry(industrySlug: string): DesignCategory {
  return INDUSTRY_DESIGN_CATEGORIES[industrySlug] || "signature";
}

// Get default preset for a design category
export function getDefaultPresetForCategory(category: DesignCategory): string {
  return DEFAULT_PRESETS[category] || "clean-blue";
}

// Check if industry has a specific design category override
export function hasDesignCategoryOverride(industrySlug: string): boolean {
