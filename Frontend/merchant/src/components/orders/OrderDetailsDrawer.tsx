...
import { Button, Drawer, Icon, Input, cn } from "@vayva/ui";
import { TrackingLinkShare } from "./TrackingLinkShare";
...
export const OrderDetailsDrawer = ({
  ...
  // Fetch shipment data when order is opened
  React.useEffect(() => {
    if (order?.id && order?.fulfillmentType === "delivery") {
      setIsLoadingShipment(true);
      apiJson<{ success: boolean; shipment: ShipmentData | null }>(`/api/orders/${order.id}/shipment`, {
        method: "GET",
      })
        .then((response) => {
        ...
        .catch((e: unknown) => {
          logger.error("[SHIPMENT_FETCH_ERROR]", {
    ...
    if (showCancel) {
      return (
        <div className="flex flex-col gap-3 w-full animate-in slide-in-from-bottom-5">
          ...
          <Input className="w-full border border-gray-100 rounded-xl p-2 text-sm bg-white   focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-green-500 focus-visible:ring-offset-2"
          ...
          order.status !== UnifiedOrderStatus.NEW && (
            <div className="flex gap-2 justify-center pt-2">
              <Button
                variant="link"
                onClick={() => setShowCancel(true)}
                className="text-xs font-bold text-gray-500 hover:text-gray-900 underline p-0 h-auto"
              >
                Cancel Order
              </Button>
      ...
      <div className="p-6 space-y-8 pb-32">
            ...
            className={cn(
              ...
              order.status === UnifiedOrderStatus.NEW
                ...
                : order.status === UnifiedOrderStatus.COMPLETED
                  ? "bg-green-500/10 text-green-500 border-green-500/20"
                  : order.status === UnifiedOrderStatus.CANCELLED
                    ? "bg-white/40 text-gray-500 border-gray-100"
                    : "bg-white/40 text-gray-500 border-gray-100",
            )}
          >
        ...
        <div className="bg-white rounded-2xl p-4 border border-gray-100  ">
          <div className="flex items-center gap-3 mb-4">
            <div className="w-10 h-10 rounded-full bg-white/40 border border-gray-100 flex items-center justify-center font-bold text-gray-500">
          ...
          <div className="flex gap-2">
            <Button className="flex-1 bg-green-500 hover:bg-green-600 text-white rounded-xl font-bold gap-2">
        ...
        {order.fulfillmentType === "delivery" && (
          <>
            <TrackingLinkShare
              ...
              storeName={(order as any).store?.name || order.customer.name}
              storeSlug={(order as any).store?.slug}
              customDomain={(order as any).store?.customDomain}
              orderId={order.id}
            />
            
            {/* Live Map for Active Deliveries */}
            {shipment?.trackingCode && ["ACCEPTED", "PICKED_UP", "IN_TRANSIT"].includes(shipment.status) && (
              <div className="bg-white rounded-2xl p-4 border border-gray-100  ">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <h4 className="font-bold text-sm text-gray-900">Live Tracking</h4>
                      <p className="text-xs text-gray-500">View rider location in real-time</p>
                    </div>
                  </div>
                  <Button
                    size="sm"
                    className="bg-green-500 hover:bg-green-600 text-white rounded-xl"
                    onClick={() => router.push(`/orders/live-tracking?order=${order.id}`)}
                  >
                ...
                <div className="bg-gradient-to-br from-green-50 to-blue-50 rounded-xl p-4">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full bg-orange-100 flex items-center justify-center animate-pulse">
                      <Icon name="Bike" size={20} className="text-orange-600" />
                    </div>
                    ...
                    <div className="text-right">
                      ...
                      <p className="font-mono font-bold text-green-600">{shipment.trackingCode}</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            {order.items.map((item: any, i: number) => (
              ...
              >
                <div className="flex gap-3">
                  <div className="w-6 h-6 rounded bg-white/40 border border-gray-100 flex items-center justify-center text-xs font-bold text-gray-500">
                    {item.quantity}x
                  </div>
                  <div>
                    <p className="text-sm font-medium text-gray-900">
                    ...
                    {item.modifiers && (
                      <div className="flex flex-wrap gap-1 mt-1">
                        {item.modifiers.map((m: any, idx: number) => (
                          <span
                            key={idx}
                            className="text-[10px] bg-white/40 text-gray-500 px-1.5 rounded border border-gray-100"
                          >
                            {m}
                          </span>
                        ))}
