...
import React /* eslint-disable @typescript-eslint/no-unused-vars */, {
  useState,
  useEffect,
  useRef,
} from "react";
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="w-80 sm:w-96 h-[500px] bg-white rounded-2xl shadow-lg overflow-hidden flex flex-col border border-gray-100"
          >
            {/* Header */}
            <div className="bg-white  p-4 text-gray-900 flex items-center justify-between border-b border-gray-100">
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 bg-white/40 rounded-full flex items-center justify-center border border-gray-100 overflow-hidden">
                  <Image
              ...
              <Button
                ...
                variant="ghost"
                size="icon"
                className="hover:bg-white/40 text-gray-500"
              >
                <X size={20} />
              </Button>
            </div>
            ...
            {messages.filter((m) => m.feedback === "NOT_SOLVED").length >=
              2 && (
              <div className="bg-white/40 p-3 border-b border-gray-100 flex items-center justify-between">
                ...
                <Button
                  ...
                  className="text-xs bg-white text-gray-900 px-3 py-1.5 rounded-lg border border-gray-100 font-medium hover:bg-white/40 transition-colors"
                >
                  Talk to Human
                </Button>
              </div>
                ...
                >
                    ...
                    <div className="mt-2 flex flex-wrap gap-2">
                      {msg.actions.map((action: string, i: number) => (
                        <Button
                          ...
                          className="text-xs bg-white/40 text-gray-500 px-2 py-1 rounded-full border border-gray-100 hover:bg-white/50 transition-colors"
                        >
                          {action}
                        </Button>
                      ))}
                  ...
                  {msg.role === "bot" && msg.messageId && (
                    <div className="mt-1 flex gap-2 ml-1">
                      {msg.feedback ? (
                        <span
                          className={`text-[10px] font-medium ${msg.feedback === "SOLVED" ? "text-gray-500" : "text-gray-400"}`}
                        >
                          {msg.feedback === "SOLVED"
                            ? "Marked as Solved"
                            : "Feedback recorded"}
            ...
            <div className="bg-white border-t border-gray-100">
              ...
              <div className="px-4 py-2 flex gap-2 overflow-x-auto no-scrollbar mask-linear-fade">
                {[
                  ...
                  <Button
                    ...
                    className="whitespace-nowrap flex-shrink-0 text-[10px] font-medium bg-white/40 text-gray-500 px-2.5 py-1.5 rounded-full border border-gray-100 hover:bg-white/50 transition-colors"
                  >
                    {chip}
                  </Button>
                ))}
              ...
              <div className="p-4 pt-2">
                <div className="flex items-center gap-2 bg-white/40 rounded-xl px-3 py-2 border border-gray-100 focus-within:border-green-500/40 focus-within:ring-2 focus-within:ring-green-500/20 transition-all">
                  <Input type="text"
                    value={input}
                    ...
                    className="flex-1 bg-transparent border-none focus:ring-0 text-sm py-1 placeholder:text-gray-400 text-gray-900"
                  />
                  <Button
                    onClick={handleSendMessage}
                    disabled={!input.trim() || isLoading}
                    className="p-1.5 bg-green-500 text-white rounded-lg disabled:opacity-50 disabled:bg-white/40 disabled:text-gray-400 hover:bg-green-600 transition-colors"
                  >
                    <Send size={16} />
                  </Button>
                </div>
