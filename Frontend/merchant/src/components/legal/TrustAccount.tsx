    ...
    <Card className="p-6 border-l-4 border-green-600 shadow-lg  bg-white/90">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-xl font-bold text-gray-900">Trust Account</h2>
        </div>
      ...
      <div className="mb-4">
        ...
        <div className="text-xs text-gray-500">Total Trust Balance</div>
      </div>
      ...
      <div className="space-y-2 mb-4 max-h-48 overflow-y-auto">
        {data.clientBalances.slice(0, 5).map((client, idx) => (
          <div key={idx} className="flex items-center justify-between text-sm">
            <span className="text-gray-500">{client.clientName}</span>
            <span className="font-medium text-gray-900">₦{client.balance.toLocaleString()}</span>
          </div>
        ))}
      </div>
      ...
      {(data.disbursementsPending > 0 || data.negativeBalanceAlerts > 0) && (
        <div className="space-y-2 pt-4 border-t border-gray-200">
          {data.disbursementsPending > 0 && (
            <div className="flex items-center gap-2 text-xs text-amber-800 bg-orange-50 p-2 rounded">
