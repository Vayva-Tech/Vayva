...
export function DynamicOnboarding() {
  ...
  const getStepIcon = (step: OnboardingStep, index: number) => {
    ...
    if (index === currentStep) {
      return (
        <div className="w-8 h-8 rounded-full bg-green-500/10 flex items-center justify-center ring-2 ring-green-500">
          <span className="text-sm font-medium text-green-500">{index + 1}</span>
        </div>
      );
    }
    return (
