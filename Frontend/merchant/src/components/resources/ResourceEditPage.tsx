...
export function ResourceEditPage({
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center h-64" aria-live="polite" role="status">
        <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-text-green-500" />
        <span className="sr-only">Loading resource data...</span>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center h-64 text-center" aria-live="assertive" role="alert">
