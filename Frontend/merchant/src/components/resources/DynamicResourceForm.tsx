...
export const DynamicResourceForm = ({
  ...
  const handleChange = (field: string, val: any) => {
    setFormData((prev: Record<string, unknown>) => ({ ...prev, [field]: val }));
    // Clear error on type
    if (errors[field]) {
  ...
  const renderInput = (key: string, required: boolean) => {
    const def = (FIELD_REGISTRY as Record<string, { label: string; type: string; placeholder?: string; options?: { value: string; label: string }[]; helpText?: string }>)[key] || { label: key, type: "text" };
      ...
      case "select":
        return (
          <Select
            ...
            className={`w-full p-2 border rounded-md ${err ? "border-red-500" : "border-gray-100"}`}
            value={String(formData[key] ?? "")}
  ...
  const renderFieldBlock = (key: string, required: boolean) => {
    ...
    return (
      <div className={isWide ? "col-span-2" : "col-span-1"}>
        ...
        {def.helpText && (
          <p className="text-xs text-gray-400 mt-1">{def.helpText}</p>
        )}
        {errors[key] && (
      ...
      <Card className="p-6">
        <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-6">
          ...
          {variantLabel && mode === "create" && (
            <div className="col-span-2 mt-4 bg-white/40 p-4 rounded-xl border border-dashed border-gray-100 text-center">
