...
export function KitchenTicket({ order, onStatusChange }: KitchenTicketProps) {
  ...
  return (
    <Card
      className={`flex flex-col h-full overflow-hidden border-2 ${isPreparing ? "border-amber-500 bg-amber-50" : "border-gray-100"}`}
    >
      {/* Header */}
      ...
      <div className="p-4 flex-1 space-y-3">
        ...
        {order.customerNote && (
          <div className="mt-4 p-2 bg-green-50 border border-accent/20 rounded text-xs text-gray-900">
      ...
      <div className="p-4 border-t bg-white/30">
        {order.fulfillmentStatus === "UNFULFILLED" ? (
          ...
          <Button
            ...
            className="w-full gap-2 bg-green-500 hover:bg-green-50 text-green-600-foreground"
            disabled={isLoading}
          >
            {isLoading ? (
              <Loader2 className="animate-spin" />
