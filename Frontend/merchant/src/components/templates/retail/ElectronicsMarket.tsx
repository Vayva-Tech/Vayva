...
interface ElectronicsItem {
  ...
  price: number;
  type: string;
  metadata?: { specs?: string; color?: string };
  image?: string;
  images?: string[];
}
...
export const ElectronicsMarketTemplate: React.FC<TemplateProps> = ({
  ...
  const electronicsItems = demoMode
    ...
    : (products as ElectronicsItem[]).filter((p) => p.type === "retail");
    ...
    <div className="font-sans min-h-screen bg-[#F8F9FA] text-[#1A1A1A]">
        ...
        <div className="container mx-auto px-6 relative z-10 flex flex-col md:flex-row items-center gap-12">
          <div className="flex-1 space-y-8 text-center md:text-left">
            ...
            <h1 className="text-5xl md:text-7xl font-black leading-none tracking-tighter">
              ELEVATE YOUR <br />
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-vayva-green to-green-400">
                TECH STACK
              </span>
            </h1>
            <p className="text-gray-400 max-w-lg text-lg leading-relaxed font-medium">
              The ultimate destination for tech enthusiasts. Discover curated,
              high-performance electronics with Vayva verified warranty.
            </p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center md:justify-start">
              <Button className="bg-vayva-green text-black hover:bg-vayva-green/90 px-10 py-7 rounded-2xl font-black text-lg shadow-xl shadow-green/20">
                Shop Now
              </Button>
              <Button
                variant="outline"
                className="border-white/20 text-white hover:bg-white/10 px-10 py-7 rounded-2xl font-black text-lg"
              >
                Learn More
              </Button>
            </div>
      ...
      <main className="container mx-auto px-6 py-20">
        <div className="flex items-center justify-between mb-12">
          ...
          <Button
            variant="ghost"
            className="font-black text-sm uppercase tracking-widest text-gray-500 hover:text-black"
          >
            View All
          </Button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {electronicsItems.map((item: ElectronicsItem) => (
            <div
              key={item.id}
              className="group bg-white rounded-[2rem] p-4 border border-gray-100 hover:border-vayva-green/30 hover:shadow-2xl hover:shadow-green/5 transition-all duration-500"
            >
              ...
              <div className="px-2 space-y-4">
                <div>
                  <h3 className="text-xl font-black mb-1 group-hover:text-vayva-green transition-colors">
                    {item.name}
                  </h3>
                ...
                <div className="flex items-center justify-between pt-4 border-t border-gray-50">
                  ...
                  <Button
                    ...
                    className="bg-black text-white hover:bg-vayva-green hover:text-black w-14 h-14 rounded-2xl transition-all duration-300 shadow-lg shadow-black/10 hover:shadow-green/20"
                  >
