    ...
    <div className={`bg-gray-50 rounded-xl border border-gray-100 ${className}`}>
      ...
      <div className="grid grid-flow-col auto-cols-fr gap-px bg-border/20 p-px">
        {columns.map((column) => (
          <div
            key={column.key}
            className="bg-white-tertiary px-4 py-3 text-xs font-medium text-gray-400 uppercase tracking-wider"
      ...
      <VirtualScroll
        ...
        renderItem={(item, index) => (
          <div className="grid grid-flow-col auto-cols-fr gap-px bg-border/20 p-px hover:bg-green-50-primary/5 transition-colors">
            {columns.map((column) => (
              <div
                key={column.key}
