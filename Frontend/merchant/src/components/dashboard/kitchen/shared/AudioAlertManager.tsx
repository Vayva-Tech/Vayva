...
const ALERT_PATTERNS: Record<AlertType, { duration: number; pattern: number[] }> = {
 ...
 */
export function AudioAlertManager({ storeId, enabled = true }: AudioAlertManagerProps) {
   ...
   */
  const playBeepPattern = useCallback((alertType: AlertType) => {
    if (!isEnabledRef.current || isMuted || !audioContextRef.current) return;
   ...
   */
  const playAudioFile = useCallback((alertType: AlertType) => {
    if (!isEnabledRef.current || isMuted) return;
   ...
   */
  const triggerAlert = useCallback((alertType: AlertType, force = false) => {
    const now = Date.now();
  ...
  useEffect(() => {
    ...
    const handleAudioAlert = (event: CustomEvent) => {
      const { type } = event.detail;
      triggerAlert(type);
    };

    window.addEventListener('kds-audio-alert', handleAudioAlert as EventListener);

    return () => {
      window.removeEventListener('kds-audio-alert', handleAudioAlert as EventListener);
    };
...
export function useAudioAlert() {
  const triggerAlert = (alertType: AlertType, force = false) => {
