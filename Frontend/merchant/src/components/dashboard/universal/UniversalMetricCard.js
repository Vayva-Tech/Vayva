 ...
 */
export function UniversalMetricCard({ title, value, change, icon: _Icon, trend, loading = false, className, designCategory: externalDesignCategory, onClick, status = 'default', 'aria-label': ariaLabel }) {
    ...
    const _getIconContainerClasses = () => {
    ...
    if (loading) {
        return (_jsx(Card, { className: getCardClasses(), children: _jsxs("div", { className: "space-y-3", children: [_jsx(Skeleton, { className: "h-3 w-16 rounded" }), _jsx(Skeleton, { className: "h-7 w-20 rounded" }), _jsx(Skeleton, { className: "h-3 w-12 rounded" })] }) }));
    }
    const cardAriaLabel = ariaLabel || `${title}: ${value}${change ? `, ${change.isPositive ? 'up' : 'down'} ${Math.abs(change.value)}%` : ''}`;
    return (_jsx(Card, { className: cn(getCardClasses(), className), onClick: onClick, role: onClick ? 'button' : 'region', tabIndex: onClick ? 0 : undefined, "aria-label": cardAriaLabel, onKeyDown: onClick ? (e) => {
            if (e.key === 'Enter' || e.key === ' ') {
                e.preventDefault();
                onClick();
            }
        } : undefined, children: _jsxs("div", { className: "space-y-3", children: [_jsx("p", { className: getTitleClasses(), children: title }), _jsx("p", { className: getValueClasses(), children: value }), (change || trend) && (_jsx("p", { className: getChangeClasses(), children: change
