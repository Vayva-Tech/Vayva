...
function SectionShell({ title, icon, children, className, designCategory }: SectionShellProps) {
  ...
  const getContainerClass = () => {
    switch (designCategory) {
      case "glass":
        return "rounded-2xl border border-gray-100 bg-white ";
      case "dark":
        return "rounded-2xl border border-gray-100 bg-white ";
      case "bold":
        return "rounded-2xl border-2 border-black bg-white";
      case "natural":
        return "rounded-2xl border border-amber-200 bg-orange-50/80 ";
      default: // signature
        return "rounded-2xl border border-gray-100 bg-white ";
    }
  };
      ...
      <div className="flex items-center gap-2 px-6 pt-6 pb-2">
        {icon && (
          <Icon
            ...
            size={16}
            className={cn(
              designCategory === "dark" ? "text-gray-400" :
...
export function PrimaryObjectHealth({
          ...
          {[1, 2, 3].map((i) => (
            <div
              ...
              className={cn(
                ...
                designCategory === "dark" ? "bg-gray-100" :
  ...
  const getItemClass = (type: 'normal' | 'warning' | 'danger' = 'normal') => {
    if (designCategory === "dark") {
      ...
      if (type === 'danger') return "border-red-200 bg-red-50";
    ...
    if (designCategory === "natural") {
      if (type === 'warning') return "border-amber-300 bg-orange-100";
      if (type === 'danger') return "border-red-300 bg-red-100";
      return "border-amber-200 bg-orange-50";
    }
    // signature/default
  ...
  const getTextClass = (type: 'primary' | 'secondary' | 'tertiary' | 'success' | 'warning' | 'danger' = 'primary') => {
    ...
    if (designCategory === "natural") {
      ...
      if (type === 'secondary') return "text-orange-700";
      if (type === 'tertiary') return "text-orange-600";
    ...
    // signature/default
    if (type === 'primary') return "text-gray-900";
    if (type === 'secondary') return "text-gray-500";
    if (type === 'tertiary') return "text-gray-400";
...
export function LiveOperations({
  ...
  const getValueClass = (isCritical: boolean) => {
    if (designCategory === "dark") {
      return isCritical ? "text-red-400" : "text-gray-900";
    }
    if (designCategory === "bold") {
      return isCritical ? "text-red-600 font-bold" : "text-gray-900";
    }
    if (designCategory === "natural") {
      return isCritical ? "text-red-700" : "text-amber-900";
    }
    // signature/default
    return isCritical ? "text-status-danger" : "text-gray-900";
  };

  const getIconClass = (isCritical: boolean) => {
    if (designCategory === "dark") {
      return isCritical ? "text-red-400" : "text-gray-400";
    }
    if (designCategory === "bold") {
      return isCritical ? "text-red-500" : "text-gray-500";
    }
    if (designCategory === "natural") {
      return isCritical ? "text-red-600" : "text-orange-600";
    }
    return isCritical ? "text-status-danger" : "text-gray-400";
  };

  const getLabelClass = () => {
    if (designCategory === "dark") return "text-gray-400";
    if (designCategory === "bold") return "text-gray-600";
    if (designCategory === "natural") return "text-orange-700";
    return "text-gray-500";
  };
...
export function AlertsList({ alerts, isLoading, designCategory }: AlertsListProps) {
    ...
    return (
      <SectionShell title="Bottlenecks & Alerts" icon="ShieldAlert" designCategory={designCategory}>
        <div className={cn(
          ...
          designCategory === "dark" ? "border-green-200 bg-green-50 text-green-600" :
          designCategory === "bold" ? "border-green-300 bg-green-50 text-green-700" :
          designCategory === "natural" ? "border-green-300 bg-green-100 text-green-700" :
          ...
          <Icon
            ...
            className={cn("shrink-0", 
              designCategory === "dark" ? "text-green-600" :
              designCategory === "bold" ? "text-green-600" :
              designCategory === "natural" ? "text-green-600" :
              "text-status-success"
            )}
          />
          <span className={cn("text-sm",
            designCategory === "dark" ? "text-gray-900" :
  ...
  const getAlertClass = (severity: string) => {
    ...
    if (designCategory === "dark") {
      if (severity === "critical") return { border: "border-red-200", bg: "bg-red-50", icon: "AlertOctagon", iconColor: "text-red-400" };
      if (severity === "warning") return { border: "border-amber-200", bg: "bg-amber-50", icon: "AlertTriangle", iconColor: "text-amber-600" };
      return { border: "border-gray-100", bg: "bg-gray-50", icon: "Info", iconColor: "text-gray-400" };
    }
    if (designCategory === "bold") {
    ...
    if (designCategory === "natural") {
      if (severity === "critical") return { border: "border-red-300", bg: "bg-red-100", icon: "AlertOctagon", iconColor: "text-red-700" };
      if (severity === "warning") return { border: "border-amber-300", bg: "bg-orange-100", icon: "AlertTriangle", iconColor: "text-orange-700" };
      return { border: "border-amber-200", bg: "bg-orange-50", icon: "Info", iconColor: "text-orange-600" };
    }
  ...
  const getTextClass = (type: 'primary' | 'secondary' = 'primary') => {
    if (designCategory === "dark") {
      return type === 'primary' ? "text-gray-900" : "text-gray-400";
    }
    if (designCategory === "bold") {
      return type === 'primary' ? "text-gray-900" : "text-gray-600";
    }
    if (designCategory === "natural") {
      return type === 'primary' ? "text-amber-900" : "text-orange-700";
    }
    return type === 'primary' ? "text-gray-900" : "text-gray-500";
  };
...
export function SuggestedActionsList({
    ...
    return (
      <SectionShell title="Suggested Actions" icon="Lightbulb" designCategory={designCategory}>
        <div className={cn(
          ...
          designCategory === "dark" ? "border-gray-100 bg-white text-gray-400" :
          designCategory === "bold" ? "border-gray-300 bg-gray-50 text-gray-600" :
          designCategory === "natural" ? "border-amber-200 bg-orange-50 text-orange-700" :
  ...
  const getActionClass = (severity: string) => {
    ...
    if (designCategory === "dark") {
      if (severity === "critical") return { border: "border-red-200", bg: "bg-red-50", hoverBg: "hover:bg-red-100" };
      if (severity === "warning") return { border: "border-amber-200", bg: "bg-amber-50", hoverBg: "hover:bg-amber-100" };
      return { border: "border-gray-100", bg: "bg-white", hoverBg: "hover:bg-gray-50" };
    }
    if (designCategory === "bold") {
    ...
    if (designCategory === "natural") {
      if (severity === "critical") return { border: "border-red-300", bg: "bg-red-100", hoverBg: "hover:bg-red-200" };
      if (severity === "warning") return { border: "border-amber-300", bg: "bg-orange-100", hoverBg: "hover:bg-amber-200" };
      return { border: "border-amber-200", bg: "bg-orange-50", hoverBg: "hover:bg-orange-100" };
    }
  ...
  const getTextClass = (type: 'primary' | 'secondary' | 'tertiary' = 'primary') => {
    if (designCategory === "dark") {
      return type === 'primary' ? "text-gray-900" : type === 'secondary' ? "text-gray-400" : "text-gray-400";
    }
    if (designCategory === "bold") {
      return type === 'primary' ? "text-gray-900" : type === 'secondary' ? "text-gray-600" : "text-gray-500";
    }
    if (designCategory === "natural") {
      return type === 'primary' ? "text-amber-900" : type === 'secondary' ? "text-orange-700" : "text-orange-600";
    }
    return type === 'primary' ? "text-gray-900" : type === 'secondary' ? "text-gray-500" : "text-gray-400";
  };
          ...
          return (
            ...
            >
              <div className={cn(
                ...
                designCategory === "dark" ? "bg-gray-100" :
                designCategory === "bold" ? "bg-gray-100" :
                designCategory === "natural" ? "bg-orange-100" :
                "bg-white-2/50"
              )}>
                <Icon
