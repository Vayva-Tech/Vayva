    ...
    <div className="space-y-4">
      ...
      <Card className="border-l-4 border-l-red-500 bg-gradient-to-br from-red-950/30 to-card rounded-2xl">
        <CardContent className="p-4">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-500 mb-1">Immediate Attention Required</p>
      ...
      <div className="space-y-2">
        {students.slice(0, 5).map((student) => {
          ...
          return (
            <Card key={student.studentId} className={`border-l-4 hover:border-green-500/50 transition-all rounded-2xl ${getColor(severity)}`}>
              <CardContent className="p-4">
                <div className="space-y-3">
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-3">
                      {getIcon(severity)}
                      <div>
                        <h4 className="font-semibold text-lg text-gray-900">{student.studentName}</h4>
