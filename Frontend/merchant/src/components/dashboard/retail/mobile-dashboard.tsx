...
export function MobileDashboard({ children, className }: MobileDashboardProps) {
  ...
  return (
    ...
    >
      {/* Refresh Indicator */}
      {isRefreshing && (
        <div className="fixed top-0 left-0 right-0 z-50 flex justify-center p-2 bg-white">
      ...
      {/* Swipe Hint */}
      <div className="fixed bottom-24 left-1/2 -translate-x-1/2 text-xs text-gray-500 opacity-50 pointer-events-none">
        ← Swipe →
      </div>
    </div>
  );
...
export function MobileListItem({
  ...
  return (
    <div
      onClick={onClick}
      className={cn(
        'flex items-center gap-3 p-4 border-b last:border-0 active:bg-green-50 transition-colors cursor-pointer',
        className
      )}
    >
      {icon && <div className="text-gray-500">{icon}</div>}
