...
export const ChatWindow = ({
  ...
  return (
    <div className="flex flex-col h-full bg-[#E5DDD5]/30 relative">
      ...
      {/* Header */}
      <header className="h-[60px] bg-white border-b border-gray-100 flex items-center px-4 justify-between shrink-0">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-full bg-border flex items-center justify-center text-gray-500 font-bold text-xs">
        ...
        <div className="flex gap-2">
          <Button
            size="icon"
            variant="ghost"
            className="text-gray-400 hover:bg-white/40"
          ...
          if (isSystem) {
            return (
              <div key={msg.id} className="flex justify-center my-4">
                <div className="bg-white/40 text-gray-400 text-xs px-3 py-1.5 rounded-full shadow-sm border border-gray-100 flex items-center gap-2">
      ...
      {/* QUICK REPLY BAR */}
      <div className="bg-white border-t border-gray-100 p-2 overflow-x-auto whitespace-nowrap flex gap-2 custom-scrollbar shrink-0">
        {/* eslint-disable-next-line @typescript-eslint/no-explicit-any */}
        {QUICK_REPLIES.map((qr) => (
          <Button
            ...
            className="rounded-full bg-white/40 text-xs text-gray-500 border-gray-100 h-8"
          >
            {qr}
          </Button>
        ))}
      ...
      <div className="p-3 bg-white flex items-end gap-2 shrink-0">
        <Button
          size="icon"
          variant="ghost"
          className="text-gray-400 hover:text-gray-500 hover:bg-white/40 rounded-full"
        ...
        <div className="flex-1 bg-white/40 rounded-xl px-4 py-2 flex items-center">
          <Input className="w-full bg-transparent border-none outline-none text-sm text-gray-900 placeholder:text-gray-400"
