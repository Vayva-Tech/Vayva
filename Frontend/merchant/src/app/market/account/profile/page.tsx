  ...
  return (
    <MarketShell>
      <div className="max-w-4xl mx-auto py-8 px-4">
        ...
        {/* Profile Info */}
        <div className="bg-white-1 border rounded-xl p-6 mb-6">
        ...
        <div className="mb-6">
          ...
          {addresses.length === 0 ? (
            <div className="text-center py-8 border rounded-lg">
              <Icon name="MapPin" size={32} className="text-gray-500 mx-auto mb-2" />
              <p className="text-gray-500">No addresses saved yet</p>
            </div>
          ) : (
            <div className="grid md:grid-cols-2 gap-4">
              {addresses.map((addr) => (
                <div key={addr.id} className={`border rounded-lg p-4 ${addr.isDefault ? "border-green-500" : ""}`}>
                  <div className="flex items-center justify-between mb-2">
                    <span className="font-medium">{addr.label}</span>
                    {addr.isDefault && <span className="text-xs bg-green-500/10 text-green-500 px-2 py-1 rounded">Default</span>}
                  </div>
