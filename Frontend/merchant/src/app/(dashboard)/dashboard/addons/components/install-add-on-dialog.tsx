...
export function InstallAddOnDialog({
  ...
  const installDependency = async (dependencyId: string) => {
    try {
      const res = await fetch('/addons/install', {
        method: 'POST',
      ...
      <div className="grid grid-cols-2 gap-4">
        {addon.provides?.pages && (
          <div className="p-4 bg-gray-100 rounded-lg">
            <FileCode className="w-5 h-5 mb-2 text-green-600" />
        ...
        {addon.provides?.components && (
          <div className="p-4 bg-gray-100 rounded-lg">
            <Sparkles className="w-5 h-5 mb-2 text-green-600" />
        ...
        {progress.map((step) => (
          <div 
            ...
            className={cn(
              ...
              step.status === 'in-progress' && "text-green-600",
