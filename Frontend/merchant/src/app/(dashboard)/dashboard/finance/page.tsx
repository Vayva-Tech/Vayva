...
function RevenueChart({
  ...
  const getX = (i: number) =>
    paddingX + (i / (revenueData.length - 1)) * chartW;
  const getY = (v: number) =>
    paddingY + chartH - ((v - minVal) / (maxVal - minVal)) * chartH;
...
function ProfitMarginChart({
  ...
  const path = profitMarginData
    .map((d, i) => `${i === 0 ? "M" : "L"} ${getX(i)} ${getY(d.margin)}`)
    .join(" ");
...
export default function FinancePage() {
      ...
      <div className="min-h-screen flex items-center justify-center">
        <div className="bg-white rounded-2xl border border-red-100 p-8 text-center max-w-md">
          ...
          <p className="text-sm text-gray-500 mb-4">
            There was a problem fetching your finance data. Please try again.
          </p>
          <Button
            onClick={() => mutate()}
      ...
      <div className="min-h-screen space-y-6">
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-sm text-gray-500 max-w-sm mb-4">
            Revenue and transactions will appear after your first sale
          </p>
        </div>
      </div>
    );
  ...
  const kpis = [
    {
      label: "Total Revenue",
      value: finKpis?.totalRevenue ?? "--",
      icon: DollarSign,
      iconBg: "bg-green-100 text-green-600",
      trend: finKpis?.revenueTrend ?? "--",
      trendUp: finKpis?.revenueTrendUp ?? true,
    },
    {
      label: "Available Balance",
      value: finKpis?.availableBalance ?? "--",
      icon: Wallet,
      iconBg: "bg-blue-100 text-blue-600",
      trend: finKpis?.balanceTrend ?? "--",
      trendUp: finKpis?.balanceTrendUp ?? true,
    },
    {
      label: "Pending Payouts",
      value: finKpis?.pendingPayouts ?? "--",
      icon: Clock,
      iconBg: "bg-amber-100 text-amber-600",
      trend: finKpis?.payoutsTrend ?? "--",
      trendUp: finKpis?.payoutsTrendUp ?? false,
    },
    {
      label: "Monthly Growth",
      value: finKpis?.monthlyGrowth ?? "--",
      icon: TrendingUp,
      iconBg: "bg-purple-100 text-purple-600",
      trend: finKpis?.growthTrend ?? "--",
      trendUp: finKpis?.growthTrendUp ?? true,
    },
  ];
          ...
          <div className="flex items-start justify-between gap-4 mb-4">
            <div>
              ...
              <p className="text-xs text-gray-500 mt-0.5">
                Your available balance, pending balance, and dedicated virtual
                account.
              </p>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <div className="lg:col-span-2">
              <AccountCard
                ...
                lines={
                  virtualAccount ? ["Send money here to fund your wallet"] : []
                }
                copyItems={
                  virtualAccount
                    ? [
          ...
          {kpis.map((kpi) => (
            ...
            >
              <div className="flex items-start justify-between mb-3">
                ...
                <div
                  className={`flex items-center gap-1 text-xs font-semibold px-2 py-1 rounded-lg ${kpi.trendUp ? "text-green-700 bg-green-50" : "text-red-600 bg-red-50"}`}
                >
                  {kpi.trendUp ? (
              ...
              {transactions.map((txn) => (
                ...
                >
                  <div className="flex items-center gap-3 min-w-0">
                    <div
                      className={`w-8 h-8 rounded-lg flex items-center justify-center ${txn.type === "Credit" ? "bg-green-50" : "bg-red-50"}`}
                    >
                      {txn.type === "Credit" ? (
            ...
            <table className="w-full">
              <thead scope="col">
                <tr className="border-b border-gray-100">
                  ...
                  <th
                    className="text-center py-3 px-6 text-xs font-semibold text-gray-500 uppercase tracking-wider"
                    scope="col"
                  >
                    Status
                  </th>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
          <div className="flex items-start justify-between gap-4 mb-4">
            <div>
              ...
              <p className="text-xs text-gray-500 mt-0.5">
                Approve or reject affiliate payout requests (these are separate
                from your own withdrawals).
              </p>
            </div>
            <Button
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
              ...
              <ExpenseDonut
                ...
                totalExpenses={finKpis?.totalExpenses ?? "₦0"}
              />
            </div>
            ...
            <div className="absolute inset-0 bg-white/70 backdrop-blur-sm rounded-2xl flex items-center justify-center z-10">
              <div className="text-center p-8 max-w-sm">
                ...
                <p className="text-sm text-gray-500 mb-5">
                  Get detailed expense breakdowns and profit margin trends to
