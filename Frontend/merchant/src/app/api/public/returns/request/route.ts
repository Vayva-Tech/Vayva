...
export async function POST(request: NextRequest) {
  try {
    const body = await request.json();
        ...
        // Validate return token via backend API
        const validateResult = await apiJson<{
          success: boolean;
          data?: { orderId?: string; customerPhone?: string };
          error?: string;
        }>(`${process.env.BACKEND_API_URL}/api/public/returns/validate-token?token=${encodeURIComponent(token)}`);
        ...
        // Create return request via backend API
        const createResult = await apiJson<{
          success: boolean;
          data?: { id?: string };
          error?: string;
        }>(`${process.env.BACKEND_API_URL}/api/public/returns/request`, {
          method: 'POST',
          headers: {
