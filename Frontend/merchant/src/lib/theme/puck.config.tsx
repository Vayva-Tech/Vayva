  ...
  components: {
    Hero: {
      ...
      render: ({ title, description, imageUrl }) => (
        <div className="py-20 bg-white/40 flex flex-col items-center justify-center text-center px-4 rounded-2xl overflow-hidden relative">
          {imageUrl && (
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {[...Array(limit || 4)].map((_, i) => (
              <div key={i} className="space-y-3 group cursor-pointer">
                <div className="aspect-[4/5] bg-white/40 rounded-2xl overflow-hidden border border-gray-100 relative shadow-sm group-hover:shadow-xl transition-all">
      ...
      render: ({ title }) => (
        <div className="py-12 space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            {[1, 2, 3].map((i: number) => (
              <div key={i} className="h-48 rounded-2xl bg-white/40 border border-gray-100 flex items-center justify-center group cursor-pointer relative overflow-hidden">
    ...
    TextBlock: {
      ...
      render: ({ text, align }) => (
        <div className={`py-12 px-4 ${align === "center" ? "text-center" :
          ...
          <p className="text-2xl md:text-3xl font-bold leading-relaxed text-gray-900 max-w-4xl mx-auto italic">
            "{text}"
          </p>
        </div>
      ),
