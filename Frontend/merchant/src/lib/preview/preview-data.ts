...
export type PreviewStore = ReturnType<typeof getPreviewStore>;
export type PreviewProduct = PreviewStore["products"][number];

export function getPreviewStore(templateSlug: string) {
  // SAFETY: Prevent demo data leakage in production unless explicitly enabled for Preview Deployment
  if (
    process.env.NODE_ENV === "production" &&
    process.env.NEXT_PUBLIC_ENABLE_DEMO_PREVIEW !== "true"
  ) {
