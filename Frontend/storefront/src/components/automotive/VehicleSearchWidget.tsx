...
import { MagnifyingGlass as Search, Car } from "@phosphor-icons/react/ssr";
...
export function VehicleSearchWidget(): React.JSX.Element {
  ...
  useEffect(() => {
    fetch("/api/vehicles?type=years")
      ...
      .catch((e) =>
        reportError(e, {
          scope: "VehicleSearchWidget.fetchYears",
          app: "storefront",
        }),
      );
  }, []);
  ...
  useEffect(() => {
    ...
    fetch(`/api/vehicles?type=makes&year=${selectedYear}`)
      ...
      .catch((e) =>
        reportError(e, {
          scope: "VehicleSearchWidget.fetchMakes",
          app: "storefront",
          selectedYear,
        }),
      )
  ...
  useEffect(() => {
    ...
    fetch(`/api/vehicles?type=models&year=${selectedYear}&make=${selectedMake}`)
      ...
      .catch((e) =>
        reportError(e, {
          scope: "VehicleSearchWidget.fetchModels",
          app: "storefront",
          selectedYear,
          selectedMake,
        }),
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        >
          {loading ? (
            ...
            <>
              <Search className="w-4 h-4 mr-2" /> Search Inventory
            </>
          )}
        </Button>
      </div>
