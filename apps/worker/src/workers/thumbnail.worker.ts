...
export function registerThumbnailWorker(connection: RedisConnection): void {
  new Worker<ThumbnailJobData>(
    ...
    async (job) => {
      ...
      try {
        await generateThumbnail({ storeId, url, deploymentId, templateProjectId });
      } catch (error: unknown) {
  ...
  logger.info("Registered thumbnail worker", { queue: QUEUES.THUMBNAIL_GENERATION, app: "worker" });
}
