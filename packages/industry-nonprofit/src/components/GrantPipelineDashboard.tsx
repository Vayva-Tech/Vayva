...
export const GrantPipelineDashboard: React.FC<GrantPipelineDashboardProps> = ({
  ...
  const pipelineStages = useMemo(() => ['research', 'draft', 'submitted', 'awarded'], []);

  return (
    <div className="bg-white rounded-lg shadow-lg p-6">
      <DashboardErrorBoundary serviceName="GrantPipelineHeader">
        <h3 className="text-xl font-semibold mb-6">💰 Grant Pipeline</h3>
      </DashboardErrorBoundary>

      {/* Summary Stats */}
      <DashboardErrorBoundary serviceName="GrantPipelineStats">
      ...
      {/* Pipeline Stages */}
      <DashboardErrorBoundary serviceName="GrantPipelineStages">
        <div className="mb-6">
      ...
      {/* Grant List - Virtualized for performance */}
      <DashboardErrorBoundary serviceName="GrantPipelineList">
        <div className="max-h-[600px] overflow-y-auto">
          <VirtualList<Grant>
            ...
            overscan={3}
            renderItem={(grant) => (
              <div key={grant.id} className="p-4 border rounded-lg hover:shadow-md transition-shadow mb-3">
                ...
                {onUpdateStatus && grant.status !== 'awarded' && grant.status !== 'rejected' && (
                  <div className="mt-3 flex gap-2">
                    <Button
                      ...
                      aria-label={`Move ${grant.name} to draft status`}
                    >
                      Move to Draft
                    </Button>
                    <Button
                      ...
                      aria-label={`Mark ${grant.name} as submitted`}
                    >
                      Mark Submitted
                    </Button>
                  </div>
