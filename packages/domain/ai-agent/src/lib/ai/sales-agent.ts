...
function getMeta(value: unknown): UnknownRecord {
...
type FulfillmentPolicy = {
  ...
  whatsappAgent?: { allowImageUnderstanding?: boolean };
...
export class SalesAgent {
  ...
  static async handleMessage(
    ...
    try {
      ...
      // 1. Pre-Check Limits
      const limitCheck = await AiUsageService.checkLimits(
        storeId,
        conversationId,
      );
      if (!limitCheck.allowed) {
        const reasonText = (limitCheck.reason || "").toLowerCase();
        const isTrialEnded =
          reasonText.includes("trial") || reasonText.includes("ended");
        const friendlyMessage = isTrialEnded
          ? "I’d love to keep chatting, but this store’s free AI trial has ended so I need to pause here. I’ve flagged this for the owner so they can top things up."
          : "I’ve hit the current AI limit for this store, so I have to pause my replies for now. I’ve let the store owner know so they can upgrade or add more messages.";
        return {
      ...
      if (trigger) {
        await EscalationService.triggerHandoff({
          ...
          trigger,
          reason: `Detected escalation trigger in customer message: ${lastMessage.substring(0, 80)}`,
          aiSummary: `Rule-based escalation (${trigger}) from last customer message.`,
        });
        return {
      ...
      const contextItems: AiContextItem[] = Array.isArray(context)
        ? (context as AiContextItem[])
        : [];
      const fulfillment = isRecord(fulfillmentPolicy)
        ? (fulfillmentPolicy as FulfillmentPolicy)
        : undefined;
      ...
      const contextString = contextItems
        .map((c: AiContextItem) => `[${c.sourceType}]: ${c.content}`)
        .join("\n");
      ...
      const storeMetadata: StoreMetadata = {
        ...
        returnPolicy: getString(settings.returnPolicy) || undefined,
        phone: supportPhone || undefined,
      };

      const pickupPoints = Array.isArray(fulfillment?.pickupPoints)
        ? fulfillment.pickupPoints
        : [];
      const pickupPointSummary = pickupPoints.length
        ? pickupPoints
            .filter((p) => Boolean(p) && (p.isDefault || p.isPickupPoint))
            .slice(0, 2)
            .map(
              (p) =>
                `${getString(p.name)}: ${[getString(p.address), getString(p.city), getString(p.state)].filter(Boolean).join(", ")}`,
            )
            .join(" | ")
        : "N/A";

      const fulfillmentSnapshot = fulfillment
        ? `FULFILLMENT POLICY SNAPSHOT:\n- pickupEnabled: ${Boolean(fulfillment.pickupEnabled)}\n- deliveryEnabled: ${Boolean(fulfillment.deliveryEnabled)}\n- deliveryMode: ${getString(fulfillment.deliveryMode) || "UNKNOWN"}\n- provider: ${getString(fulfillment.deliveryProvider) || "UNKNOWN"}\n- allowImageUnderstanding: ${Boolean(fulfillment.whatsappAgent?.allowImageUnderstanding)}\n- defaultPickupPoints: ${pickupPointSummary}`
        : "FULFILLMENT POLICY SNAPSHOT: unavailable";

      const normalizedProfile: MerchantProfile | null = profile
        ? {
            agentName: profile.agentName || undefined,
            tonePreset: profile.tonePreset || undefined,
            brevityMode: profile.brevityMode || undefined,
            persuasionLevel:
              typeof profile.persuasionLevel === "number"
                ? profile.persuasionLevel
                : undefined,
          }
        : null;
      ...
      if (choice.tool_calls) {
        ...
        for (const tool of choice.tool_calls) {
          if (tool.function.name === "get_inventory") {
            ...
            const args = JSON.parse(tool.function.arguments);
            const result = await MerchantBrainService.getDeliveryQuoteV2(
              storeId,
              args.location,
            );
            toolResults.push({
            ...
            const result =
              await MerchantBrainService.getStoreFulfillmentPolicy(storeId);
            toolResults.push({
              role: "tool",
              tool_call_id: tool.id,
              content: JSON.stringify(result),
            ...
            const args = JSON.parse(tool.function.arguments);
            const result = await MerchantBrainService.describeImage(
              storeId,
              args.imageUrl,
            );
            toolResults.push({
          ...
          } else if (tool.function.name === "visual_search") {
            const args = JSON.parse(tool.function.arguments);
            ...
            const args = JSON.parse(tool.function.arguments);
            const result = await MerchantBrainService.searchCatalog(
              storeId,
              args.query,
            );
            toolResults.push({
            ...
            const result =
              await MerchantBrainService.getActivePromotions(storeId);
            toolResults.push({
              role: "tool",
              tool_call_id: tool.id,
              content: JSON.stringify(result),
          ...
          // Food & Restaurant tools
            ...
            const args = JSON.parse(tool.function.arguments);
            const result = await this.checkOrderStatus(
              storeId,
              args.orderNumber,
            );
            toolResults.push({
          ...
          // Travel & Hospitality tools
            ...
            const args = JSON.parse(tool.function.arguments);
            const result = await this.checkAccommodationAvailability(
              storeId,
              args,
            );
            toolResults.push({
          ...
          // B2B tools
            ...
            const args = JSON.parse(tool.function.arguments);
            const result = await this.getWholesalePricing(
              storeId,
              args.productId,
              args.quantity,
            );
      ...
      const retrievedDocIds = contextItems
        .map((c: AiContextItem) => c.sourceId)
      ...
      if (
        ...
        NotificationService.sendMilestone("first_sale", {
      ...
      if (choice.content?.includes("[HANDOFF_REQUIRED]")) {
        ...
        await EscalationService.triggerHandoff({
          ...
          trigger,
          reason:
            "AI agent determined this conversation should be handed to a human.",
          aiSummary: `Model-requested escalation after last customer message: ${lastMessage.substring(0, 80)}`,
        });
        return {
          message: this.getHandoffCopy(trigger),
          data: {
...
`
    ...
    const toneGuides: Record<string, string> = {
      Friendly:
        "Be warm, use casual language, add occasional emojis (1-2 max). Sound like a helpful friend.",
      Professional:
        "Be polite and efficient. Use proper grammar but stay approachable.",
      Luxury:
        "Be elegant and refined. Use sophisticated language. Make the customer feel special.",
      Playful:
        "Be fun and energetic! Use emojis freely. Keep things light and engaging.",
...
2. If the last user message contains IMAGE_META with an imageUrl:
   - If allowImageUnderstanding is true:
     a. For product matching: call visual_search(imageUrl) to find visually similar products
     b. For general description: call describe_image(imageUrl) to get image details
...
Remember: You're helping a real person. Be genuine, be helpful, be human.`;
  ...
  static detectEscalationTrigger(text: string) {
    ...
    const includesAny = (terms: string[]) =>
      terms.some((t) => lowerText.includes(t));
    // Clear fraud / scam signals
    if (includesAny([" scam", "fraud", "fraudulent", "fake", "419"]))
      return "FRAUD_RISK";
    ...
    // Strong negative sentiment or explicit frustration
    if (
      includesAny([
        "i am angry",
        "i'm angry",
      ...
      return "SENTIMENT";
    // Direct request for a human in a heated context
    if (
      includesAny([
        "speak to a human",
        "real person",
  ...
  static getHandoffCopy(trigger: string) {
    switch (trigger) {
      ...
      case "BILLING_ERROR":
        return "I'm really sorry this payment looks confusing. I've asked a human on our billing team to step in and review it with you directly.";
      case "FRAUD_RISK":
        return "I understand this feels worrying. I've flagged this for a human on our team to review urgently and respond to you directly.";
      case "SENTIMENT":
        return "I don't want to make this more frustrating. I've asked a human teammate to take over this chat and respond to you personally.";
      default:
        return "I've passed this conversation to a human teammate so they can follow up with you directly.";
    }
  }
  ...
  static async getAvailableTables(storeId: string, date: string) {
    try {
      ...
      const availableTables = tables
        .filter((t: { id: string }) => !bookedTableIds.has(t.id))
        .map((t: any) => {
          const meta = getMeta(t.metadata);
          return {
            id: t.id,
            name: t.title,
            type: getString(meta.tableType) || "Standard",
            capacity: typeof meta.capacity === "number" ? meta.capacity : 6,
            minimumSpend: Number(meta.minimumSpend) || Number(t.price) || 0,
      ...
      if (availableTables.length === 0) {
        return {
          available: false,
          message: "No tables available for this date. Try another date.",
        };
      }
      ...
      return {
        available: false,
        message: "Unable to check table availability right now.",
      };
    }
  }
    ...
    try {
      ...
      if (bottles.length === 0) {
        ...
        return {
          available: false,
          message:
            "Bottle menu not configured. Contact the venue directly for bottle service.",
        };
      }

      const menu = bottles.map((b) => {
        const meta = getMeta(b.metadata);
        return {
          id: b.id,
          name: b.title,
          price: Number(b.price),
          category: getString(meta.category) || "Spirits",
          volume: getString(meta.volume) || "",
      ...
      return {
        available: false,
        message: "Unable to load bottle menu right now.",
      };
    }
  }
  ...
  static async createReservation(
    ...
    try {
      ...
      const tableMetadata = getMeta(table.metadata);
      const minimumSpend =
        Number(tableMetadata.minimumSpend) || Number(table.price) || 0;

      // Calculate bottle total if pre-ordering
      let bottleTotal = 0;
      const bottleDetails: UnknownRecord[] = [];
      ...
      return {
        ...
        nextSteps:
          "The venue will confirm your reservation shortly. You'll receive a confirmation message.",
      };
    } catch (error) {
      ...
      return {
        success: false,
        message:
          "Unable to complete reservation. Please try again or contact the venue directly.",
      };
    }
  }
  ...
  static async getUpcomingEvents(storeId: string) {
    try {
      ...
      const upcomingEvents = events
        .filter((e) => {
          const meta = getMeta(e.metadata);
          const eventDateRaw = meta.eventDate;
          const eventDate =
            typeof eventDateRaw === "string" || eventDateRaw instanceof Date
        ...
        .map((e) => {
          ...
          return {
            id: e.id,
            name: e.title,
            date: getString(meta.eventDate),
            time: getString(meta.eventTime) || "10:00 PM",
            venue: getString(meta.venue),
            description: e.description?.substring(0, 100) || "",
            dressCode: getString(meta.dressCode),
            ticketTypes: e.productVariants.map(
              (v: { title: string; price: unknown }) => ({
                name: v.title,
    ...
    const baseTools = [
      ...
      },
      {
        ...
        function: {
          ...
          description:
            "Get store pickup/delivery capabilities, pickup points, and delivery mode (Kwik vs Manual)",
      ...
      },
      {
        ...
        function: {
          name: "describe_image",
          description:
            "Describe what is in an image URL sent by a customer (use only if allowImageUnderstanding is true).",
          parameters: {
            type: "object",
      ...
      },
      {
        ...
        function: {
          name: "visual_search",
          description:
            "Search for products by image URL. Finds visually similar products using AI vision. Returns matching products with confidence scores.",
          parameters: {
            type: "object",
            properties: {
              imageUrl: {
      ...
      },
      {
        ...
        function: {
          ...
          description:
            "Calculate delivery fee and ETA for a location, including dynamic platform margin (capped) and whether delivery is chargeable (Kwik only)",
          parameters: {
            type: "object",
            properties: { location: { type: "string" } },
            required: ["location"],
      ...
      return [
        ...baseTools,
        {
          ...
          function: {
            ...
            description:
              "Get available tables for reservation. Use when customer wants to book a table.",
            parameters: {
              type: "object",
              properties: {
        ...
        },
        {
          ...
          function: {
            ...
            description:
              "Create a table reservation. Collect: name, phone, date, table type, party size first.",
            parameters: {
              type: "object",
              properties: {
      ...
      return [
        ...baseTools,
        {
          ...
          function: {
            name: "get_services",
            description:
              "Get list of available services with prices and durations.",
            parameters: { type: "object", properties: {} },
          },
        },
        {
        ...
        },
        {
          ...
          function: {
            ...
            description:
              "Book an appointment. Collect: name, phone, service, date, time first.",
            parameters: {
              type: "object",
              properties: {
      ...
      return [
        ...
        },
        {
          ...
          function: {
            name: "place_order",
            description:
              "Place a food order. Collect: items, delivery address, phone first.",
            parameters: {
              type: "object",
              properties: {
                customerName: { type: "string" },
      ...
      return [
        ...
        },
        {
          ...
          function: {
            ...
            description:
              "Schedule a property viewing. Collect: name, phone, property, preferred date/time.",
            parameters: {
              type: "object",
              properties: {
                customerName: { type: "string" },
      ...
      return [
        ...
        },
        {
          ...
          function: {
            ...
            description:
              "Schedule a test drive. Collect: name, phone, vehicle, preferred date/time.",
            parameters: {
              type: "object",
              properties: {
                customerName: { type: "string" },
      ...
      return [
        ...
        },
        {
          ...
          function: {
            name: "book_stay",
            description:
              "Book accommodation. Collect: name, phone, dates, room type, guests first.",
            parameters: {
              type: "object",
              properties: {
                guestName: { type: "string" },
      ...
      return [
        ...
        },
        {
          ...
          function: {
            name: "enroll_course",
            description:
              "Enroll in a course. Collect: name, phone, email, course first.",
            parameters: {
              type: "object",
              properties: {
      ...
      return [
        ...
        },
        {
          ...
          function: {
            name: "request_quote",
            description:
              "Request a quote for bulk order. Collect: company, contact, items, quantities.",
            parameters: {
              type: "object",
              properties: {
      ...
      return [
        ...
        },
        {
          ...
          function: {
            name: "make_donation",
            description:
              "Process a donation. Collect: name, email, amount, campaign.",
            parameters: {
              type: "object",
              properties: {
      ...
      return {
        ...
        services: services.map((s) => {
          ...
          return {
            ...
            duration: meta.duration_min
              ? `${Number(meta.duration_min)} mins`
              : "60 mins",
            category: getString(meta.category) || "General",
          };
        }),
      };
    } catch (error) {
  ...
  static async getAvailableSlots(
    ...
    try {
      ...
      if (slots.length === 0) {
        return {
          available: false,
          message: `No available slots for ${date}. Try another date.`,
        };
      }
  ...
  static async bookAppointment(
    ...
    try {
      const { customerName, customerPhone, serviceId, date, time, notes } =
        data;
      ...
      const meta = getMeta(service.metadata);
      const durationMin = Number(meta.duration_min) || 60;
  ...
  static async getMenu(storeId: string) {
    try {
      ...
      items.forEach((item) => {
        ...
        const meta = getMeta(item.metadata);
        categories[cat].push({
          id: item.id,
          name: item.title,
          description: item.description?.substring(0, 50) || "",
          price: `₦${Number(item.price).toLocaleString()}`,
          prepTime: meta.prep_time ? `${Number(meta.prep_time)} mins` : "",
        });
      });
  ...
  static async placeOrder(
    ...
    try {
      ...
      let total = 0;
      const orderItems: UnknownRecord[] = [];
      ...
      if (orderItems.length === 0) {
        return {
          success: false,
          message: "No valid items found. Please check the menu.",
        };
      }
      ...
      const order = await prisma.booking.create({
        data: {
          storeId,
          serviceId: (orderItems[0]?.productId as string) || "unknown_service", // Use first item as service reference
          startsAt: new Date(),
  ...
  static async checkOrderStatus(storeId: string, orderNumber: string) {
    try {
      ...
      const itemsArr = Array.isArray(meta.items) ? meta.items : [];
      ...
      return {
        ...
        items: itemsArr.map((i) => {
          const rec = getMeta(i);
          return `${getString(rec.quantity)}x ${getString(rec.name)}`;
        }),
        total:
      ...
      return {
        ...
        properties: filtered.map((p) => {
          ...
          return {
            ...
            bedrooms: typeof meta.rooms === "number" ? meta.rooms : 0,
            bathrooms: typeof meta.bathrooms === "number" ? meta.bathrooms : 0,
            area: meta.area ? `${Number(meta.area)} sqm` : "",
  ...
  static async searchVehicles(
    ...
    try {
      ...
      const filtered = vehicles.filter((v) => {
        ...
        if (
          filters.make &&
          !vp.make?.toLowerCase().includes(filters.make.toLowerCase())
        )
          return false;
        if (
          filters.model &&
          !vp.model?.toLowerCase().includes(filters.model.toLowerCase())
        )
          return false;
        if (filters.minYear && vp.year && vp.year < filters.minYear)
          return false;
        if (filters.maxYear && vp.year && vp.year > filters.maxYear)
          return false;
        if (
          filters.condition &&
          vp.condition !== filters.condition.toUpperCase()
        )
          return false;
        return true;
      });
      ...
      return {
        ...
        vehicles: filtered.map((v) => ({
          ...
          mileage: v.vehicleProduct?.mileage
            ? `${v.vehicleProduct.mileage.toLocaleString()} km`
            : "",
  ...
  static async checkAccommodationAvailability(
    ...
    try {
      ...
      const available = rooms.filter((r) => {
        ...
        if (roomType && !r.title.toLowerCase().includes(roomType.toLowerCase()))
          return false;
        return true;
      });
    ...
    try {
      ...
      const room = await prisma.product.findFirst({
        where: {
          ...
          status: "ACTIVE",
          ...(roomType && {
            title: { contains: roomType, mode: "insensitive" },
          }),
        },
        select: { id: true, title: true, price: true },
      });
      ...
      return {
        ...
        courses: courses.map((c) => {
          ...
          return {
            ...
            instructor: getString(meta.instructor),
          };
        }),
      };
    } catch (error) {
  ...
  static async enrollCourse(
    ...
    try {
      ...
      return {
        ...
        nextSteps:
          "Complete payment to access the course. You'll receive payment instructions shortly.",
      };
    } catch (error) {
  ...
  static async getWholesalePricing(
    storeId: string,
    productId: string,
    quantity: number,
  ) {
  ...
  static async requestQuote(
    ...
    try {
      ...
      // Calculate quote
      const quoteItems: UnknownRecord[] = [];
      let total = 0;
      ...
      const booking = await prisma.booking.create({
        data: {
          storeId,
          serviceId: (quoteItems[0]?.productId as string) || "unknown_service", // Use first item as reference
          startsAt: new Date(),
      ...
      return {
        ...
        campaigns: campaigns.map((c) => {
          ...
          return {
            ...
            raised:
              typeof meta.raised === "number"
                ? `₦${meta.raised.toLocaleString()}`
                : "₦0",
            cause: getString(meta.cause),
          };
        }),
      };
    } catch (error) {
  ...
  static async makeDonation(
    ...
    try {
      ...
      return {
        ...
        nextSteps:
          "You'll receive payment instructions to complete your donation.",
      };
    } catch (error) {
