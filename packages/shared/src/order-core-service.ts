        ...
        return await prisma.$transaction(async (tx) => {
            ...
            const order = await tx.order.create({
                data: {
                    ...
                    metadata: {
                        ...(typeof (cart as unknown as Record<string, unknown>)?.metadata === "object" ? (cart as unknown as Record<string, unknown>).metadata as Record<string, unknown> : {}),
                        channel: "MARKETPLACE",
                        importStatus: isImportOrder ? String(ImportOrderState.CREATED) : null,
                    },
                }
            });
