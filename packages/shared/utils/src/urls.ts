...
export const urls = {
  marketingBase: () =>
    process.env.MARKETING_ORIGIN || must("MARKETING_BASE_URL"),
  ...
  webstudioBase: () =>
    process.env.WEBSTUDIO_BASE_URL || "https://apps.webstudio.is",
  ...
  merchantDashboard: () => `${urls.merchantBase()}/dashboard`,
  ...
  merchantResetPasswordUrl: (token: string) =>
    urls.merchantResetPassword(token),
