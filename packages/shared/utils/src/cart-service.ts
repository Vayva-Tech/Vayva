...
export class CartService {
  static async getCart(cartId: string): Promise<EnrichedCart | null> {
    ...
    const resultGroups: SplitCartGroup[] = Object.values(groups).map(
      (group) => {
        ...
        if (settings && settings.isEnabled) {
          canDeliver = true;
          // StoreDeliverySettings doesn't have baseDeliveryFee/deliveryFeeType fields
          // Use default flat fee for now
          fee = 1000;
        } else {
          fee = 1000;
          canDeliver = true;
        ...
        group.items.forEach((item) => {
          ...
          const productMetadata = isRecord(product.metadata)
            ? product.metadata
            : {};
          const depositRequired = productMetadata.depositRequired === true;
          const depositPercentage =
            typeof productMetadata.depositPercentage === "number"
              ? productMetadata.depositPercentage
              : null;
          groupPayable +=
  ...
  static async addItem(
    ...
    variantId: string,
    quantity: number = 1,
  ): Promise<Prisma.CartItemGetPayload<object> | null> {
