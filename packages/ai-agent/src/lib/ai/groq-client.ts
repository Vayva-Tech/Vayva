...
export class GroqClient {
     ...
     */
    async chatCompletion(messages: unknown[], options: ChatCompletionOptions = {}): Promise<Chat.ChatCompletion | null> {
        if (!(this.client as unknown as { apiKey?: unknown }).apiKey || (this.client as unknown as { apiKey?: unknown }).apiKey === "placeholder-key") {
            logger.warn("[GroqClient] Call skipped due to missing API key");
            return null;
        }
        try {
            // 1. Sanitize user messages
            const safeMessages: Chat.ChatCompletionMessageParam[] = (messages as Chat.ChatCompletionMessageParam[]).map((m) => {
