...
export async function getRedis(config: RedisConfig = {}): Promise<Redis> {
  ...
  if (isBuildTime()) {
    console.warn(
      "⚠️ Redis initialization attempted during build time. Returning a proxy to prevent connection.",
    );
    ...
    return new Proxy({} as Redis, {
      get(_, prop) {
        ...
        return () => {
          throw new Error(
            `❌ Redis method "${String(prop)}" called during build time or Redis is not configured.`,
          );
        };
      },
    });
  ...
  try {
    ...
    instance.on("error", (err) => {
      const now = Date.now();
      if (now - lastRedisErrorLogAtMs < 60000) return;
