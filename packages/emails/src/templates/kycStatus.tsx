...
export function KycStatusEmail(props: {
  ...
  return (
    <VayvaGlowLayout
      ...
      headline={isApproved ? "Your identity has been verified" : "Your KYC submission needs attention"}
      body={
        isApproved ? (
          <>
            Great news! The identity verification for{" "}
            <strong style={{ color: "#E8FFF2" }}>{props.storeName}</strong> has been approved.
            You can now receive payouts and access all platform features.
          </>
        ) : (
          <>
            The identity verification for{" "}
            <strong style={{ color: "#E8FFF2" }}>{props.storeName}</strong> could not be completed.
            {props.reason && (
              <>
